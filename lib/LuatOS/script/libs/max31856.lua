...
--Note: The configuration of ads1115 needs to be configured according to project requirements. You need to reconfigure ADS1115_CONF_HCMD and ADS1115_CONF_LCMD according to the configuration register instructions!!!
--Usage examples
...
max31856.TCTYPE_B                   =   0x00    --Type B
max31856.TCTYPE_E                   =   0x01    --Type E
max31856.TCTYPE_J                   =   0x02    --Type J
max31856.TCTYPE_K                   =   0x03    --Type K
max31856.TCTYPE_N                   =   0x04    --TypeN
max31856.TCTYPE_R                   =   0x05    --TypeR
max31856.TCTYPE_S                   =   0x06    --Type S
max31856.TCTYPE_T                   =   0x07    --Type T

max31856.ONESHOT                    =   0x00    --Single conversion mode
max31856.CONTINUOUS                 =   0x01    --automatic conversion mode

max31856.SAMPLE1                    =   0x00    --1 sample
max31856.SAMPLE2                    =   0x01    --2 samples
max31856.SAMPLE4                    =   0x02    --4 samples
max31856.SAMPLE8                    =   0x03    --8 samples
max31856.SAMPLE16                   =   0x04    --16 samples

local MAX31856_CR0_REG   	        =   0x00    --Configuration register 0
local MAX31856_CR1_REG       	    =   0x01    --Configuration register 1
local MAX31856_MASK_REG      	    =   0x02    --fault mask register
local MAX31856_CJHF_REG      	    =   0x03    --Cold end upper limit fault
local MAX31856_CJLF_REG      	    =   0x04    --Cold connection lower limit fault
local MAX31856_LTHFTH_REG      	    =   0x05    --Linearization Temperature High Limit Fault MSB
local MAX31856_LTHFTL_REG      	    =   0x06    --Linearized temperature upper limit fault LSB
local MAX31856_LTLFTH_REG      	    =   0x07    --Linearization temperature lower limit fault MSB
local MAX31856_LTLFTL_REG      	    =   0x08    --Linearization temperature lower limit fault LSB
local MAX31856_CJTO_REG      	    =   0x09    --Cold junction temperature offset register
local MAX31856_CJTH_REG      	    =   0x0A    --Cold junction temperature register MSB
local MAX31856_CJTL_REG      	    =   0x0B    --Cold junction temperature register LSB
local MAX31856_LTCBH_REG      	    =   0x0C    --Linearized TC temperature, byte 2
local MAX31856_LTCBM_REG      	    =   0x0D    --Linearized TC temperature, byte 1
local MAX31856_LTCBL_REG      	    =   0x0E    --Linearized TC temperature, byte 0
local MAX31856_SR_REG      	        =   0x0F    --fault status register
...
@usage
max31856.set_tc_type(max31856.TCTYPE_K) --Set type to K
]]
...
@usage
max31856.set_avgsel(max31856.SAMPLE1) --Set averaging mode to 1 sample
]]
...
@usage
max31856.set_cmode(max31856.ONESHOT) --Set conversion mode to single conversion
]]
...
function max31856.init(spi_device)
    ...
    max31856_write_cmd(MAX31856_MASK_REG, 0x00) --Assert all errors
    max31856_write_cmd(MAX31856_CR0_REG, 0x10) --Enable open circuit fault detection
    max31856_write_cmd(MAX31856_CJTO_REG, 0x00) --Cold junction temperature offset set to zero
