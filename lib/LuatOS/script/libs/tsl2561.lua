...
sys.taskInit(function()
    ...
    tsl2561.init(i2cid)--Initialization, pass in i2c_id
    while 1 do
...
--The last two digits set the integration time, NOMINAL INTEGRATION TIME 13.7ms 101ms 402ms
local TSL2561_TIMING_13MS	      =   0x00	--Integration time 13.7ms
local TSL2561_TIMING_101MS	      =   0x01  --Integration time 101ms
local TSL2561_TIMING_402MS	      =   0x02  --Integration time 402ms

local TSL2561_TIMING_GAIN_1X      =   0x00 --Gain 1
local TSL2561_TIMING_GAIN_16X     =   0x10 --Gain 16 times

--Device ID detection
local function chip_check()
    i2c.send(i2cid, TSL2561_ADDRESS_ADR_LOW, TSL2561_CHIP_ID_CHECK)--Read device address
    ...
    if revData:byte() ~= nil then
        ...
        i2c.send(i2cid, TSL2561_ADDRESS_ADR_HIGH, TSL2561_CHIP_ID_CHECK)--Read device address
        sys.wait(50)
        local revData = i2c.recv(i2cid, TSL2561_ADDRESS_ADR_HIGH, 1)
        if revData:byte() ~= nil then
            TSL2561_ADDRESS_ADR = TSL2561_ADDRESS_ADR_HIGH
        else
            i2c.send(i2cid, TSL2561_ADDRESS_ADR_FLOAT, TSL2561_CHIP_ID_CHECK)--Read device address
            sys.wait(50)
    ...
    sys.wait(50)
    i2c.send(i2cid, TSL2561_ADDRESS_ADR, TSL2561_CHIP_ID_CHECK)--Read device address
