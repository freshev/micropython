...
sys.taskInit(function()
    ...
    ak8963.init(i2cid)--Initialization, pass in i2c_id
    while 1 do
        sys.wait(100)
        local mag = ak8963.get_mag()--Get a geomagnetometer
...
local function ak8963_check()
    i2c.send(i2cid, AK8963_ADDRESS_AD0_LOW, AK8963_RA_WHO_AM_I)--Read device address
    sys.wait(50)
    local revData = i2c.recv(i2cid, AK8963_ADDRESS_AD0_LOW, 1)
    if revData:byte() ~= nil then
        i2cslaveaddr = AK8963_ADDRESS_AD0_LOW
    else
        i2c.send(i2cid, AK8963_ADDRESS_AD0_HIGH, AK8963_RA_WHO_AM_I)--Read device address
        sys.wait(50)
    ...
    i2c.send(i2cid, i2cslaveaddr, AK8963_RA_WHO_AM_I)--Read device address
    sys.wait(50)
...
--Get raw data from geomagnetometer
