...
local rc522_idle              =       0x00   --Cancel current command
local rc522_authent           =       0x0E   --Verification key
local rc522_receive           =       0x08   --receive data
local rc522_transmit          =       0x04   --Send data
local rc522_transceive        =       0x0C   --Send and receive data
local rc522_resetphase        =       0x0F   --reset
local rc522_calccrc           =       0x03   --CRC calculation

--Mifare_One card command word
rc522.reqidl                 =       0x26   --It does not enter the dormant state in the antenna area.
rc522.reqall                 =       0x52   --Search all cards in the antenna area
local rc522_anticoll1        =       0x93   --Anti-collision
local rc522_anticoll2        =       0x95   --Anti-collision
local rc522_authent1a        =       0x60   --Verify A key
local rc522_authent1b        =       0x61   --Verify B key
local rc522_read             =       0x30   --Read block
local rc522_write            =       0xA0   --write block
local rc522_decrement        =       0xC0   --Deduction
local rc522_increment        =       0xC1   --top up
local rc522_restore          =       0xC2   --Transfer block data to buffer
local rc522_transfer         =       0xB0   --Save data in buffer
local rc522_halt             =       0x50   --hibernate

--MF522 register definition
--PAGE 0
local rc522_rfu00            =       0x00 --Reserved for future use
local rc522_com_mand         =       0x01 --Start and stop command execution
local rc522_com_ie           =       0x02 --Enable and disable control bits for interrupt request delivery
local rc522_divl_en          =       0x03 --Enable and disable control bits for interrupt request delivery
local rc522_com_irq          =       0x04 --Contains interrupt request flag
local rc522_div_irq          =       0x05 --Contains interrupt request flag
local rc522_error            =       0x06 --Error flag indicating the error status of the last command executed
local rc522_status1          =       0x07 --Contains status flags for communication
local rc522_status2          =       0x08 --Contains status flags for the receiver and transmitter
local rc522_fifo_data        =       0x09 --64-byte FIFO buffer input and output
local rc522_fifo_level       =       0x0A --Indicates the number of bytes stored in the FIFO
local rc522_water_level      =       0x0B --Define FIFO depth for FIFO underflow and overflow alarms
local rc522_control          =       0x0C --different control registers
local rc522_bit_framing      =       0x0D --Bit-oriented frame tuning
local rc522_coll             =       0x0E --Bit position of the first bit collision detected on the RF interface
local rc522_rfu0f            =       0x0F --Reserved for future use
--PAGE 1
local RFU10                  =       0x10 --Reserved for future use
local rc522_mode             =       0x11 --Define common patterns for sending and receiving
local rc522_tx_mode          =       0x12 --Define the data transfer rate for the sending process
local rc522_rx_mode          =       0x13 --Define the data transfer rate during reception
local rc522_tx_control       =       0x14 --Logic characteristics that control antenna driver pins TX1 and TX2
local rc522_tx_ayto          =       0x15 --Control antenna driver settings
local rc522_tx_sel           =       0x16 --Select internal source for antenna driver
local rc522_rx_sel           =       0x17 --Select internal receiver settings
local rc522_rx_threshold     =       0x18 --Select bit decoder threshold
local rc522_demod            =       0x19 --Define demodulator settings
local rc522_rfu1a            =       0x1A --Reserved for future use
local rc522_rfu1b            =       0x1B --Reserved for future use
local rc522_mifare           =       0x1C --Controls communication at 106kbit/s in ISO 14443/MIFARE mode
local rc522_rfu1d            =       0x1D --Reserved for future use
local rc522_rfu1e            =       0x1E --Reserved for future use
local rc522_serial_speed     =       0x1F --Select the speed of the serial UART interface
--PAGE 2
local rc522_rfu20            =       0x20 --Reserved for future use
local rc522_crcresult_m      =       0x21 --Displays the actual MSB value of the CRC calculation
local rc522_crcresult_l      =       0x22 --Displays the actual LSB value of the CRC calculation
local rc522_rfu23            =       0x23 --Reserved for future use
local rc522_mod_width        =       0x24 --Control the setting of ModWidth
local rc522_rfu25            =       0x25 --Reserved for future use
local rc522_rfcfg            =       0x26 --Configure receiver gain
local rc522_gsn              =       0x27 --Select the modulation conductance of the antenna driver pins TX1 and TX2
local rc522_cwgscfg          =       0x28 --Select the modulation conductance of the antenna driver pins TX1 and TX2
local rc522_modgscfg         =       0x29 --Select the modulation conductance of the antenna driver pins TX1 and TX2
local rc522_tmode            =       0x2A --Define the settings for the internal timer
local rc522_tprescaler       =       0x2B --Define the settings for the internal timer
local rc522_tpreload_h       =       0x2C --Description 16-bit timer reload value
local rc522_tpreload_l       =       0x2D --Description 16-bit timer reload value
local rc522_tcounter_value_h =       0x2E --Description 16-bit timer reload value
local rc522_tcounter_value_l =       0x2F --Description 16-bit timer reload value
--PAGE 3
local rc522_rfu30            =       0x30 --Reserved for future use
local rc522_testsel1         =       0x31 --Configuration of commonly used test signals
local rc522_testsel2         =       0x32 --Configuration and PRBS control of common test signals
local rc522_testpin_en       =       0x33 --D1-D7 output driver enable pins (Note: only for serial interface)
local rc522_testpin_value    =       0x34 --Define the values   of D1-D7 when used as I/O bus
local rc522_testbus          =       0x35 --Displays the status of the internal test bus
local rc522_autotest         =       0x36 --Control digital self-test
local rc522_version          =       0x37 --show version
local rc522_analogtest       =       0x38 --Control pins AUX1 and AUX2
local rc522_testadc1         =       0x39 --Define the test value for TestDAC1
local rc522_testadc2         =       0x3A --Define test values   for TestDAC2
local rc522_testadc          =       0x3B --Display actual values   of ADC I and Q channels
local rc522_rfu3c            =       0x3C --Reserved for product testing
local rc522_rfu3d            =       0x3D --Reserved for product testing
local rc522_rfu3e            =       0x3E --Reserved for product testing
local rc522_rfu3f		     =       0x3F --Reserved for product testing
