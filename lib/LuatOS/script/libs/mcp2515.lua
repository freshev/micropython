...
-- CANCTRL 
mcp2515.REQOP_CONFIG    = 0x80--configuration mode
mcp2515.REQOP_LISTEN    = 0x60--Listen mode
mcp2515.REQOP_LOOPBACK  = 0x40--Loopback mode for testing
mcp2515.REQOP_SLEEP     = 0x20--sleep mode
mcp2515.REQOP_NORMAL    = 0x00--normal mode
mcp2515.ABORT           = 0x10
...
function mcp2515.send_buffer(config,...)
    if config.ide then
        mcp2515.write(mcp2515.TXB0SIDH,(config.id>>21)&0xFF)--Transmit buffer 0 standard identifier high bit
        mcp2515.write(mcp2515.TXB0SIDL,((config.id>>16)&0x03)|0x08|((config.id>>13)&0xE0))--The low bits of the standard identifier of send buffer 0 and the highest two bits of the extended identifier of buffer 0 (the third bit is the enable bit of the extended identifier for sending)
        mcp2515.write(mcp2515.TXB0EID8,(config.id>>8)&0xFF)--Transmit buffer 0 marks the high bit of the extended identifier
        mcp2515.write(mcp2515.TXB0EID0,config.id&0xFF)--Transmit buffer 0 marks the low bit of the extended identifier
    else
        mcp2515.write(mcp2515.TXB0SIDH,(config.id>>3)&0xFF)--Transmit buffer 0 standard identifier high bit
        mcp2515.write(mcp2515.TXB0SIDL,(config.id&0x07)<<5)--Transmit buffer 0 standard identifier low bit
    end
    if select("#",...)>8 then
    ...
    mcp2515.write(mcp2515.TXB0D0,...)--Write the data to be sent into the send buffer register
    if config.rtr then
        mcp2515.write(mcp2515.TXB0DLC,select("#",...)|0x40)--Write the length of data to be sent in this frame to the send length register of send buffer 0
    else
        mcp2515.write(mcp2515.TXB0DLC,select("#",...))--Write the length of data to be sent in this frame to the send length register of send buffer 0
    end
    mcp2515.write(mcp2515.TXB0CTRL,0x08)--Request to send message
end
...
function mcp2515.filter(id,ide,shield)
    mcp2515.mode(mcp2515.REQOP_CONFIG)--Enter configuration mode
    if ide then
        if shield then
            mcp2515.write(mcp2515.RXM0SIDH,(id>>21)&0xFF)--Configure acceptance mask register n standard identifier high bit
	        mcp2515.write(mcp2515.RXM0SIDL,((id>>16)&0x03)|((id>>13)&0xE0))--Configure acceptance mask register n standard identifier low bit
            mcp2515.write(mcp2515.RXM0EID8,(id>>8)&0xFF)--Configure the acceptance mask register n extended identifier high bit
	        mcp2515.write(mcp2515.RXM0EID0,id&0xFF)--Configure acceptance mask register n extended identifier low bit
        else
            mcp2515.write(mcp2515.RXF0SIDH,(id>>21)&0xFF)--Configure acceptance filter register n standard identifier high bit
            mcp2515.write(mcp2515.RXF0SIDL,((id>>16)&0x03)|0x08|((id>>13)&0xE0))--Configure the low bit of the acceptance filter register n standard identifier (the third bit is the receive extended identifier enable bit)
            mcp2515.write(mcp2515.RXF0EID8,(id>>8)&0xFF)--Configure acceptance filter register n standard identifier high bit
	        mcp2515.write(mcp2515.RXF0EID0,id&0xFF)--Configure acceptance filter register n standard identifier low bit
        end
    else
        if shield then
            mcp2515.write(mcp2515.RXM0SIDH,(id>>3)&0xFF)--Configure acceptance mask register n standard identifier high bit
	        mcp2515.write(mcp2515.RXM0SIDL,(id&0x07)<<5)--Configure acceptance mask register n standard identifier low bit
        else
            mcp2515.write(mcp2515.RXF0SIDH,(id>>3)&0xFF)--Configure acceptance filter register n standard identifier high bit
	        mcp2515.write(mcp2515.RXF0SIDL,(id&0x07)<<5)--Configure the low bit of the acceptance filter register n standard identifier (the third bit is the receive extended identifier enable bit)
        end
    end
    mcp2515.mode(mcp2515.REQOP_NORMAL)--Enter normal mode
end
...
function mcp2515.init(spi_id,cs,int,baud)
    ...
    mcp2515.reset()
    --The following parts are modified according to the requirements of the reference manual.
    --Configure CNF1, CNF2, CNF3,
    mcp2515.baud(baud)
	mcp2515.write(mcp2515.CNF2,0x80|mcp2515.PHSEG1_3TQ|mcp2515.PRSEG_1TQ)
	mcp2515.write(mcp2515.CNF3,mcp2515.PHSEG2_3TQ)
	mcp2515.write(mcp2515.RXB0SIDH,0x00)--Clear the standard identifier high bits of receive buffer 0
	mcp2515.write(mcp2515.RXB0SIDL,0x00)--Clear the standard identifier low bits of receive buffer 0
    mcp2515.write(mcp2515.RXB0EID8,0x00)--Clear the extended identifier high bit of receive buffer 0
	mcp2515.write(mcp2515.RXB0EID0,0x00)--Clear the low bits of the extended identifier of receive buffer 0
	mcp2515.write(mcp2515.CANINTF,0x00)--Clear all bits of the CAN interrupt flag register (must be cleared by the MCU)
	mcp2515.write(mcp2515.CANINTE,0x01)--Configure the CAN interrupt enable register to enable the receive buffer 0 full interrupt and disable interrupts in other bits.
    mcp2515.mode(mcp2515.REQOP_NORMAL)--Enter normal mode
    return true
end
