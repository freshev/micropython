...
local nfc_work=false--Anti-collision

--LPCD configuration information
local LPCD_THRSH_H = 0x00
local LPCD_THRSH_L = 0x10
local LPCD_CWP = 10 --LPCD PMOS output driver 0~63
local LPCD_CWN = 10 --LPCD NMOS output driver 0~15
local LPCD_SLEEPTIME = 10 --LPCD wake-up interval, each level is 32ms, sleep time: (16+1)*32=544ms
...
local function SetReg(address, value)
    ...
    --log.info("Register writing: 0x",string.format("%X",address),"The value is 0x",string.format("%X",value))
...
local function GetReg(address, len)
    ...
    --log.info("Register reading: 0x",string.format("%X",address),"The value is 0x",string.format("%X",string.byte(val)))
    return string.byte(val)
end

local function SetReg_Ext(address, value)
    --log.info("Extended register write: 0x",string.format("%X",address),"The value is 0x",string.format("%X",string.byte(value)))
...
-- //***********************************************
--//Function name: GetReg_Ext(unsigned char ExtRegAddr)
--//Function: read extended register value
--//Entry parameters: ExtRegAddr: extended register address
--//Export parameters: unsigned char TRUE: read successfully FALSE: failed
-- //***********************************************
local function GetReg_Ext(address)
    --log.info("Extended register read: 0x",string.format("%X",address))
...
-- //*************************************
--//Function name: fm17622_Initial_ReaderA
--Set A card mode
--//Entry parameters:
--//Export parameters:
-- //*************************************
...
-- *************************************
--Function name: ReaderA_CardActivate
--Start looking for cards, find cards and select cards
--Entry parameters:
--Exit parameters: fm17622_SUCCESS, fm17622_COMM_ERR
-- *************************************
...
-- //***********************************************
--//Function name: Lpcd_Get_ADC_Value()
--//Function: Lpcd_Get_ADC_Value reads the ADC data of LPCD
--lpcd parameters for debugging
--//Entry parameters:
--//Export parameters:
-- //***********************************************
...
-- //***********************************************
--//Function name: Lpcd_Card_Event()
--//Function: LPCD detects card
--//Entry parameters:
--//Export parameters:
-- //***********************************************
...
local function Lpcd_Card_Event()
    ...
    nfc_Data = {}--End of clearing
end

-- //***********************************************
--//Function name: Lpcd_IRQ_Event()
--//Function: LPCD interrupt processing
--//Entry parameters:
--//Export parameters:
-- //***********************************************
function fm17622.Lpcd_IRQ_Event()
    ...
    sys.wait(5)
    local reg = GetReg_Ext(JREG_LPCDIRQ) --Read the LPCD interrupt flag. The LPCD interrupt needs to determine whether the LPCD IRQ RXCHANGE (0x04) is set.
