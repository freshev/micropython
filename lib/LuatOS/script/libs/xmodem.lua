...
local SOH           =   0x01    --Modem data header 128
local STX           =   0x02    --Modem data header 1K
local EOT           =   0x04    --End of sending
local ACK           =   0x06    --answer
local NAK           =   0x15    --No response
local CAN           =   0x18    --Cancel sending
local CTRLZ         =   0x1A    --filling
local CRC_CHR       =   0x43    --C: ASCII character C
local CRC_SIZE      =   2
...
@number uart_id uart端口号
@usage
--After performing xmodem transfer, regardless of whether the transfer is successful or not, it is recommended to close the xmodem context and also close uart.
xmodem.close(2)
]]
