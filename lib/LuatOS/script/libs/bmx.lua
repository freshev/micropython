...
sys.taskInit(function()
    ...
    bmx.init(i2cid)--Initialization, pass in i2c_id
    while 1 do
...
local BMP180_CTRL_ADDR		        =   0xF4 --control register
local BMP180_CTRL_TEMP		        =   0x2E --Conversion temperature 4.5MS
local BMP180_CTRL_POSS0		        =   0x34 --Convert atmospheric pressure 4.5ms
local BMP180_CTRL_POSS1		        =   0x74 --Convert atmospheric pressure 7.5ms
local BMP180_CTRL_POSS2		        =   0xB4 --Convert atmospheric pressure 13.5ms
local BMP180_CTRL_POSS3		        =   0xF4 --Convert atmospheric pressure 25.5ms

local BMP180_AD_MSB			        =   0xF6 --ADC output high 8 bits
local BMP180_AD_LSB			        =   0xF7 --ADC output lower 8 bits
local BMP180_AD_XLSB			    =   0xF8 --When measuring 19 bits, the ADC outputs the lowest 3 bits
...
local BMX280_CTRLMHUM_ADDR		    =   0xF2 --control register
local BMX280_STATUS_ADDR		    =   0xF3 --status register
local BMX280_CTRLMEAS_ADDR		    =   0xF4 --control register
local BMX280_CONFIG_ADDR		    =   0xF5 --configuration register
...
--Configure a mode according to the official manual, you can modify it yourself here
...
local BMX180_CTRL_TEMP		        =   0x2E --Conversion temperature 4.5MS
local BMX180_CTRL_POSS0		        =   0x34 --Convert atmospheric pressure 4.5ms
local BMX180_CTRL_POSS1		        =   0x74 --Convert atmospheric pressure 7.5ms
local BMX180_CTRL_POSS2		        =   0xB4 --Convert atmospheric pressure 13.5ms
local BMX180_CTRL_POSS3		        =   0xF4 --Convert atmospheric pressure 25.5ms
...
local PRESSURE_OF_SEA			    =   101325	--Reference sea level pressure
...
local function chip_check()
    i2c.send(i2cid, BMX_ADDRESS_ADR_LOW, BMX_CHIP_ID_CHECK)--Read device address
    local revData = i2c.recv(i2cid, BMX_ADDRESS_ADR_LOW, 1)
    if revData:byte() ~= nil then
        BMX_ADDRESS_ADR = BMX_ADDRESS_ADR_LOW
    else
        i2c.send(i2cid, BMX_ADDRESS_ADR_HIGH, BMX_CHIP_ID_CHECK)--Read device address
        sys.wait(50)
    ...
    i2c.send(i2cid, BMX_ADDRESS_ADR, BMX_CHIP_ID_CHECK)--Read device address
    sys.wait(50)
...
function bmx.init(i2c_id)
    ...
    sys.wait(20)
    if chip_check() then
        i2c.send(i2cid, BMX_ADDRESS_ADR, {BMX_RESET,BMX_RESET_VALUE})--soft reset
        sys.wait(20)
...
local function bmx_get_temp_raw()
    ...
    if CHIP_ID == BMP180_CHIP_ID then
        ...
        local buffer = i2c.recv(i2cid, BMX_ADDRESS_ADR, 2)--Get 2 bytes
