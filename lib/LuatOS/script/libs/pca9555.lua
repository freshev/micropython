...
local PCA9555_mode=0x0000 --IO is all output
local PCA9555_IOstart=0xffff

--pca9555 registers define
local PCA9555_REG_IN0 = 0x00                  --Input port register 0. The default value is determined by external logic. It can only be read but not writable.
local PCA9555_REG_IN1 = 0x01                  --Input port register 1. The default value is determined by external logic. It can only be read and cannot be written.
local PCA9555_REG_OUT0 = 0x02                 --Output port register 0 default value is 1, readable and writable
local PCA9555_REG_OUT1 = 0x03                 --Input port register 1. The default value is 1. It can be read and written.
local PCA9555_REG_POL0 = 0x04                 --Polarity Inversion Register 0 Default value is 0 Readable and writable
local PCA9555_REG_POL1 = 0x05                 --Polarity Inversion Register 1 Default value is 0 Readable and writable
local PCA9555_REG0_CFG0 = 0x06                --Configuration port register 0 defaults to 1 and is readable and writable
local PCA9555_REG1_CFG1 = 0x07                --Configuration port register 1 defaults to 1 and is readable and writable
...
@number mode 配置输入输出模式  --0xffff High eight bits configure IO 17-10 Low eight bits configure IO 7-0
@return 初始化失败，返回nil
@usage
    i2c.setup(0, i2c.FAST)
