...
// mmap cannot be used without VFS enabled
...
#endif
  // Be sure to turn it on when debugging
#if LUAT_UNDUMP_DEBUG
  LLOGD("code in ram");
...
LClosure *luaU_undump(lua_State *L, ZIO *Z, const char *name) {
  ...
  //Reset offset data
  ptr_offset = 0;
  ptr_offset ++; // The previous method has read one byte
  if (*name == '@' || *name == '=')
    S.name = name + 1;
  ...
  if (*name == '@') {
    // Since it is loaded from a file, it can be used as the source file name in the debugging information
  ...
  //Print the memory consumption of each part
#if LUAT_UNDUMP_DEBUG
