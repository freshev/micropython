...
--Message queue for data sending
local msgQuene = {}
	...
	return sys.taskInit(function()
		...
		while true do
			if libgnss.isFix() then
				...
				--log.info("Send location information.locRpt", lat, lng)
...
local cmds =
{
    [JT808Prot.S_COMMON_RSP] = s_commonRsp,  --Platform general response
    [JT808Prot.S_SET_PARA] = s_setPara,   --Set terminal parameters
    [JT808Prot.S_CONTROL] = s_control,     --terminal control
}

local tCache = {}
--tcp receive data processing function
function inproc(data)
	table.insert(tCache, data)
	local cacheData = table.concat(tCache) --connection string
	tCache = {}
	...
	while true do
		...
		while result do
			...
			if rx_buff:used() > 0 then
				...
				inproc(rx_buff:toStr(0, rx_buff:used()))  --Read out the received data and process it
				rx_buff:del()
			end
			while #msgQuene>0 do
