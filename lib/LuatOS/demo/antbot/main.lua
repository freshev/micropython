...
--Introduce ebike application example Modules
local bot_app_ebike = require("bot_app_ebike")

--antbot.app_sta_get fixed return value type
local bot_msg_type = {
...
sys.taskInit(function()
    ...
    while 1 do
        ...
        log.info("lua", rtos.meminfo())         --memory
        log.info("sys", rtos.meminfo("sys"))    --sysmemory
        count = count + 1
        ...
        if bot_msg == bot_msg_type.BOT_MSG_INIT_SUCESS then
            ...
            if antbot.asset_status_get(asset_config.id) == 1 then
                ...
                --The registration interval needs to be greater than 2s
                --sys.wait(2 * 1000)
            end
