...
local function record_cb(id, buff)
    if buff then
        ...
        codec.encode(encoder, rx_buff, amr_buff)		--Perform AMR encoding on the recording data. If successful, this interface will return true. The default encoding level is MR475.
...
local function record_task()
    ...
    sys.wait(6000)
    i2c.send(0, es8311_address,{0x00, 0x80},1)			--ES8311 stops recording
    log.info("录音5秒结束")
	...
	if result then
		...
		log.info("音频播放结果", result)				--Wait for the audio to finish playing
	else
														--Audio playback error
	end
	i2c.close(0)
	gpio.setup(26, 0)								--After recording, turn off the power of mic and es8311, and turn off i2c
	...
	while true do
	 sys.wait(2000)
	   record_cnt = 0                                  --The recording starts clearing the amr data to store the buffer data, setting the cursor position, setting the recording time to zero, deleting the file system recording file, and turning on the power of the mic and es8311.
	   amr_buff:clear(0)
	   amr_buff:seek(0)
	   os.remove(recordPath)
	   gpio.setup(26, 1)				
