...
local samplerate = 16000	--It can only be used by SOCs that support internal AMR encoding.
...
local function record_cb(id, buff)
    if buff then
		...
		if samplerate > 8000 then
			if record_cnt >= 50 then	--Stop after more than 5 seconds
				log.info("I2S", "stop") 
				i2s.stop(i2s_id)	
			end
		else
...
local function record_task()
	...
	pm.power(pm.LDO_CTL, true)							--Turn on the power supply of es8311 chip
	--Adaptive development board, the board you make yourself does not need to use both I2C
	...
	i2s.setup(i2s_id, 1, samplerate, 16, 1, i2s.MODE_LSB)			--Turn on i2s
