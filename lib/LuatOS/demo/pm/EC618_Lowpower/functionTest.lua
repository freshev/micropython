local server_ip = "112.125.89.8"    --Change it to your own. The demo uses a UDP server. If you use a TCP server, you currently need to disconnect the server before using the extreme power consumption mode.
local server_port = 34352 --Replace it with your own
local period = 1800000 --The default mode changes every 30 minutes
...
local function testTask(ip, port)
	...
	socket.config(netc, nil, true, nil) --The demo uses a UDP server. If you use a TCP server, you currently need to disconnect the server first when using extreme power consumption mode.
