...
--[[
这是使用BLE功能模拟KT6368A的demo, 从机模式, UART1透传, 并开启wifi功能
...
sys.taskInit(function()
    sys.wait(500)
    wlan.init()
    wlan.connect("uiot", "czcjhp1985cbm", true)
    ...
    --wlan.connect("uiot", "czcjhp1985cbm", true)
    -- sys.waitUntil("IP_READY")
    --log.info("wlan initialization completed")
end)
...
sys.taskInit(function ()
    while 1 do
        ...
        -- nimble.deinit()
        sys.wait(1000)
        local code = http.request("GET", "http://httpbin.air32.cn/get?abc=123").wait()
        log.info("http", code)
    end
end)
