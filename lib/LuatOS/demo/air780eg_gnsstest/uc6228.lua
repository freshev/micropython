...
function uc6228.start()
    ...
    --Do I need to switch the positioning system?
    if opts.sys then
...
local function do_agps()
    --First, initiate a location query
    local lat, lng
    if mobile then
    ...
    --Then, determine the ephemeris time and download the ephemeris
    local now = os.time()
    ...
    if not lat or not lng then
        ...
        return --TODO Do not write the reference position temporarily
    end
    if socket.sntp then
        socket.sntp()
...
function uc6228.agps(force)
    --If the AGPS information is not forced to be written and the positioning has been successful, then there is no need.
    if not force and libgnss.isFix() then return end
    --Judge the time first
    local now = os.time()
