...
--The following demonstrates using blocking mode to create an automatic answering server, which is only suitable for W5500
local dName = "D2_TASK"
...
local function serTask(port)
	...
	while true do
		...
		log.info(result)
		if result then
            result,_ = socket.accept(netc, nil)    --Only supports 1 to 1
            if result then
			    log.info("客户端连上了")
            end
		end
		...
		while result do
			...
			if rx_buff:used() > 0 then
				...
				tx_buff:copy(nil,rx_buff)   --Return the received data to the client
				rx_buff:del()
				cnt = cnt + 1
			end 
...
--if not result then
--log.info("Client disconnected", result, param)
-- 				break
-- 			end
--succ, param, rIP, rPort = socket.rx(netc, rx_buff)
--if not succ then
--log.info("Client disconnected", succ, paramt)
-- 				break
-- 			end
--if rx_buff:used() > 0 then
--remote_ip:copy(0, rIP) --for subsequent processing, necessary
--if remote_ip[0] == 0 then
--rIP = remote_ip[1] .. "." .. remote_ip[2] .. "." .. remote_ip[3] .. "." .. remote_ip[4] --not necessary, just print it for easy viewing look
--log.info("Received client data, length", rx_buff:used(), rIP, rPort)
-- 				else
--log.info("Received client data, length", rx_buff:used()) --w5500 temporarily supports IPV4 and will not handle IPV6 situations.
-- 				end
--tx_buff:copy(nil,rx_buff) --Return the received data to the client
-- 				rx_buff:del()
-- 			end 
...
-- 				else
--result = true --w5500 temporarily supports IPV4 and will not handle IPV6 situations.
-- 				end
-- 			end
--if not result then
--log.info("Sending failed", result, param)
-- 				break
-- 			end
-- 			tx_buff:del()
--if tx_buff:len() > 1024 then
...
function SerDemo(port)
	...
	--sysplus.taskInitEx(clientTask, d1Name, netCB, port) -- loopback test, you can comment it out if not needed
end
