...
local function socketTask()
	...
	socket.config(netc, nil, nil, nil, 300, 5, 6)   --Enable TCP keepalive to prevent the operator from disconnecting if there is no data interaction for a long time.
    while true do
...
local function UDPTask()
    local netc = socket.create(nil, netCB)
    socket.debug(netc, true)
    socket.config(netc, nil, true, nil, 300, 5, 6)   --Enable TCP keepalive to prevent the operator from disconnecting if there is no data interaction for a long time.
    while true do
...
local function SSLTask()
    local netc = socket.create(nil, netCB)
    socket.debug(netc, true)
    socket.config(netc, nil, nil, true, 300, 5, 6)   --Enable TCP keepalive to prevent the operator from disconnecting if there is no data interaction for a long time.
    while true do
