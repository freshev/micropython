...
#define OTA_CHECK_BUFF_SIZE (64) // If it exceeds 64 bytes, luat_md5 will report an error and needs to be checked.
...
int luat_ota_checkfile(const char* path) {
    ...
    #endif
    // There should be 16 bytes of md5
...
int luat_ota(uint32_t luadb_addr){
    ...
    //Check whether there is a compressed upgrade file
...
_close_decompress:
        ...
        //Delete it regardless of success or failure to avoid executing it every time it is started.
        luat_fs_remove(UPDATE_TGZ_PATH);
    }
#endif

    int ret  = -1;
    //Check whether there is an upgrade file
