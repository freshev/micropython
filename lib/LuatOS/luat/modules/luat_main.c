...
static LUAT_RT_RET_TYPE l_timer_error_cb(LUAT_RT_CB_PARAM) {
  LLOGE("Main.lua not found, please flash the script to run the program, luatos quick start tutorial: https://wiki.luatos.com/boardGuide/roadmap.html");
...
int luat_main_demo() { // This is to verify whether the most basic message/timer/Task mechanism of LuatVM is normal.
...
static int pmain(lua_State *L) {
    ...
    lua_gc(L, LUA_GCSETPAUSE, 90); //Set the `garbage collector interval rate` to be lower than 100%
...
/**
 * Regular process, single entry, execution script.
 * require "sys"
 *
 * ... user code ....
 *
...
int luat_main (void) {
  ...
  #endif
  /// Support time-independent compilation, consistent with idempotence
...
#endif
  // The following will definitely not be executed.
  return 0;
}
