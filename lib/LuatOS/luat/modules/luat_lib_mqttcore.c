/*@Modules libmqtt
@summary mqtt protocol processing, for use by mqtt.lua
@version 1.0
@date 2020.07.03*/
#include "luat_base.h"
#include "luat_sys.h"
...
static int l_mqttcore_packCONNECT(lua_State *L) {
	...
	cleanSession = 1; // Temporarily force clear
	...
	//Then calculate the total length, pitfalls
...
static int l_mqttcore_packSUBSCRIBE(lua_State *L) {
	...
	while (lua_next(L, 3) != 0) {
       	/*Using 'key' (at index -2) and 'value' (at index -1)*/
...
static int l_mqttcore_packPUBLISH(lua_State *L) {
	...
	//Add payload, here it is >A not >P
...
static int l_mqttcore_unpack(lua_State *L) {
	...
	// First, get the length of the package
	size_t dlen = 0;
	size_t poffset = 1;
	size_t multiplier = 1;
	...
	// Then parse the 0th byte, header data
	...
	lua_pushinteger(L, poffset+dlen+2);//Lua's index starts from 1, so you need to add an additional 1
	return 2;
}
