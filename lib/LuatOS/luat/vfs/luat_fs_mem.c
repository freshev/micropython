...
{
    size_t size;     //The current file size is also the size of the memory block corresponding to the pointer
    // size_t ptr_size; // Size of numeric pointer
...
FILE* luat_vfs_ram_fopen(void* userdata, const char *filename, const char *mode) {
    ...
    if (!strcmp("w", mode) || !strcmp("wb", mode) || !strcmp("w+", mode) || !strcmp("wb+", mode) || !strcmp("r+", mode) || !strcmp("rb+", mode)) {
        // First check if it exists, if it exists, reuse the old one
        for (size_t i = 0; i < RAM_FILE_MAX; i++)
        {
            if (files[i]== NULL)
                continue;
