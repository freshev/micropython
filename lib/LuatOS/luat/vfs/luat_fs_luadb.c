...
_after_head:
    ...
    // Since luadb_fs_t brings a luadb_file_t element
    ...
    for (size_t i = 0; i < filecount; i++)
    {
        ...
        // Just skip the check code.
        index += len;
        
        fs->files[i].ptr = (const char*)(index + ptr); // absolute address
...
// This global variable is set for bsp itself, do not change the name.
...
int luat_vfs_luadb_info(void* userdata, const char* path, luat_fs_info_t *conf) {
    ...
    // Estimate the offset of the first file of luadb as the starting position
    //The offset of the last file + file size, as the end position
    //To estimate the actual usage of luadb
    size_t used = 0;
    luadb_fs_t* fs = (luadb_fs_t*)userdata;
