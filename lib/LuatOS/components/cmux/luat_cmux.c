...
static void uih_dbg_manage(unsigned char*buff,size_t len){
    ...
    // memset(data, 0, len-3); // Make sure the padding is 0
...
//0 successful parsing 1 parsing header -1 parsing error discarded
...
next_parse:
    ...
    while (start < cmux_buff_offset) {
        ...
        // int ret = 0; // Because there is no luat_cmux_parse
        // end = cmux_buff_offset;// 
        if (ret == 0) {
            // Read ok, it is a complete frame, let luat_cmux_exec execute according to the frame format
            //Use luat_cmux_read2 as exec
        ...
        // Wait for luat_cmux_read2 to be changed and then jump out according to the situation.
        // break;
    }
    memset(cmux_buff,0,CMUX_BUFFER_SIZE);
    cmux_buff_offset =  0;
