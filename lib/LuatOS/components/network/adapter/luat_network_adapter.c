		...
		if (NW_WAIT_TX_OK == ctrl->wait_target_state)
		{
			...
			if (ctrl->ack_size == ctrl->tx_size)
			{
				...
				if (ctrl->is_tcp)
				{
					...
					{
						// If the adapter comes with the check_ack function, call it directly without calling the global one.
						...
						// TODO After all bsps have added the check_ack function, the following call will no longer be needed.
...
static int32_t network_default_socket_callback(void *data, void *param)
{
	...
	if (event->ID > EV_NW_TIMEOUT)
	{
		...
		else
		{
			...
			// There will be problems with printing the following line on some platforms
			//The reason is that ctrl may have been released, and accessing it again will cause coredump
...
//Check whether the IP is invalid, return 0 if invalid
...
//Check whether the IP is a valid IPV4 type, if not return 0
