...
int luat_websocket_payload(char *buf, luat_websocket_pkg_t *pkg, size_t limit)
{
	...
	// The payload length of websocket supports 3 situations:
	// 0 bytes (less than 126, placed in the header)
	// 2 bytes 126 ~ 0xFFFF
	// 6 bytes 0x1000 ~ 0xFFFFFFFF, not intended to be supported.
	if (pkg->plen == 126)
	{
		if (limit < 4)
		{
			// There is still 1 byte missing, wait.
	...
	{
		// It will take 8 bytes later, but this package is too big!!!
...
int luat_websocket_set_connopts(luat_websocket_ctrl_t *websocket_ctrl, luat_websocket_connopts_t* opts)
{
	...
	// TODO supports basic authorization URL ws://wendal:123@wendal.cn:8080/abc
...
int luat_websocket_send_frame(luat_websocket_ctrl_t *websocket_ctrl, luat_websocket_pkg_t *pkg)
{
	...
	//Add mark, change TODO to random?
	char mark[] = {0, 1, 2, 3};
...
static int websocket_parse(luat_websocket_ctrl_t *websocket_ctrl)
{
	...
	if (websocket_ctrl->websocket_state == 0)
	{
		...
		{ // You have to wait at least 5 characters
			LLOGD("wait more data offset %d", websocket_ctrl->buffer_offset);
			return 0;
		}
		// The first 3 characters must be 101, otherwise it must be illegal
		...
		for (size_t i = 4; i < websocket_ctrl->buffer_offset; i++)
		{
			...
			{
				// LLOGD("Found \\r\\n\\r\\n");
				// Found it!! But it seems like there is no need to deal with it at all.
				websocket_ctrl->buffer_offset = 0;
				LLOGI("ready!!");
	...
	// Determine the data length. The first few bytes can determine whether it is enough to read the websocket header.
	...
	case 0x02: // binary frame
		break;
	case 0x08:
		// Actively disconnect? I'll wipe it
