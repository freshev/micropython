...
//	uint8_t active_flag;
	...
	// LLOGD("CALL net_lwip2_task event %08X adapter_index %d", event.ID, adapter_index);
...
static void net_lwip2_check_network_ready(uint8_t adapter_index)
{
	...
	if (prvlwip.netif_network_ready[adapter_index] == active_flag) {
		LLOGD("Network [%d] status has not changed, skip check", adapter_index);
		return;
	}
	prvlwip.netif_network_ready[adapter_index] = active_flag;
	if (!active_flag)
	{
		// TODO If there is no active netif, the dns client should be closed
	...
	else
	{
		...
		if (prvlwip.lwip_netif[adapter_index] != NULL && !ip_addr_isany(&prvlwip.lwip_netif[adapter_index]->gw)) {
			NET_DBG("Use gateway as default DNS server");
			net_lwip2_set_dns_server(0, &prvlwip.lwip_netif[adapter_index]->gw, (void*)adapter_index);
		}
		else {
			NET_DBG("Use 223.5.5.5 as default DNS server");
...
static uint8_t net_lwip2_check_ready(void *user_data)
{
	...
	// LLOGD("lwip queries network readiness %d", prvlwip.netif_network_ready[adapter_index]);
	if (prvlwip.lwip_netif[adapter_index] == NULL) {
...
int net_lwip2_set_static_ip(luat_ip_addr_t *ip, luat_ip_addr_t *submask, luat_ip_addr_t *gateway, luat_ip_addr_t *ipv6, void *user_data)
{
	// uint8_t index = (uint32_t)user_data;
	// if (index >= NW_ADAPTER_INDEX_LWIP_NETIF_QTY) return -1;
	// if (!prvlwip.lwip_netif) return -1;
