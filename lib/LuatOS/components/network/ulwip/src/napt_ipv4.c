...
IPv4 version of NAPT, supporting TCP/UDP/ICMP protocols
Original code source:
https://gitee.com/openLuat/luatos-soc-air101
https://www.winnermicro.com/html/1/156/158/558.html

A lot of modifications have been made to adapt to the Luatos environment, breaking away from platform dependencies.*/
#include "luat_base.h"
#include <stdio.h>
#include <string.h>
#include "lwip/ip.h"
                         ...
                         ip_addr_t* gw_ip)
{
    ...
    if (is_inet)
    {
        ...
        return 0; // The transformation has been completed and you can return
    }
    else
    {
#ifdef NAPT_TABLE_MUTEX_LOCK
                        ...
                        ip_addr_t* gw_ip)
{
    ...
    if (is_inet)
    {
        ...
        if (NULL == napt)
        {
            ...
            // LLOGD("Assign new TCP mapping ip %d port %d -> %d", src_ip, tcp_hdr->src, napt->new_port);
        }
        else
        {
            luat_napt_table_update_4tcp(napt);
            // LLOGD("Reuse the old TCP mapping ip %d port %d -> %d", src_ip, tcp_hdr->src, napt->new_port);
        }
        memcpy(napt->mac, ptr + 6, 6); //Save the source mac address
        // LLOGD("Record the source MAC of the internal network-->the external network %02X%02X%02X%02X%02X%02X", napt->mac[0], napt->mac[1], napt->mac[2], napt->mac[3], napt->mac[4], napt->mac[5]);
    ...
    else
    {
        ...
        if (NULL != napt)
        {
            ...
            memcpy(ptr, napt->mac, 6); //Restore MAC address
            return 0;
        }
                        ...
                        ip_addr_t* gw_ip)
{
    ...
    if (is_inet)
    {
        ...
        memcpy(napt->mac, ptr + 6, 6); //Copy source MAC address
...
int luat_napt_input(u8 is_inet, u8 *pkt_body, u32 pkt_len, ip_addr_t* gw_ip)
{
    ...
    switch(ip_hdr->_proto)
    {
        ...
        case IP_PROTO_GRE:/* vpn */
        {
            ...
            if (err)
                LLOGD("GRE package rc %d", err);
            break;
        }
        default:
        {
            LLOGW("Unrecognized protocol package 0x%02X", ip_hdr->_proto);
            err = -1;
            break;
        }
    }
