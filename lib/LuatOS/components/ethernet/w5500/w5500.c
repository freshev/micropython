...
enum
{
	...
	EV_W5500_LINK,
	//The order of the following events cannot be changed. Please add new events above.
	EV_W5500_SOCKET_TX,
...
typedef struct
{
	...
	void *task_handle;
	HANDLE Sem;
	uint32_t static_ip; //Storage in big endian format
	uint32_t static_submask; //Storage in big endian format
	uint32_t static_gateway; //Storage in big endian format
	uint16_t RTR;
	uint8_t RCR;
	uint8_t force_arp;
	uint8_t auto_speed;
...
static void w5500_xfer(w5500_ctrl_t *w5500, uint16_t address, uint8_t ctrl, uint8_t *data, uint32_t len)
{
	...
	// BSP does not support full-duplex, read and write through half-duplex API
	// Send the 3-byte control block first
	// char* tmp = (char* )w5500->tx_buf;
	// LLOGD("First control block %02X%02X%02X", tmp[0], tmp[1], tmp[2]);
	if (ctrl & is_write) {
		luat_spi_send(w5500->spi_id, (const char* )w5500->tx_buf, len + 3);
	}
	else {
		luat_spi_send(w5500->spi_id, (const char* )w5500->tx_buf, 3);
		// LLOGD("Read data length %d", len);
...
static uint8_t w5500_socket_state(w5500_ctrl_t *w5500, uint8_t socket_id)
{
	...
	do
	{
		...
		if (temp[W5500_SOCKET_MR] == 0xff)	//The Modules cannot be read
		{
			w5500->device_on = 0;
			return 0;
		}
...
static void w5500_nw_state(w5500_ctrl_t *w5500)
{
	...
	if (w5500->link_ready && w5500->ip_ready)
	{
		if (!w5500->network_ready)
		{
			...
			w5500->socket[0].tx_wait_size = 0;	//dns can continue to be sent
			w5500_callback_to_nw_task(w5500, EV_NW_STATE, 0, 1, w5500->self_index);
			LLOGD("network ready");
			w5500_nw_state_cb(W5500_IP_READY, w5500->dhcp_client.temp_ip == 0 ? w5500->static_ip : w5500->dhcp_client.temp_ip);
	...
	else
	{
		if (w5500->network_ready)
		{
			...
			w5500_nw_state_cb(W5500_IP_LOSE, 0);
...
static void w5500_check_dhcp(w5500_ctrl_t *w5500)
{
	...
	if (DHCP_STATE_CHECK == w5500->dhcp_client.state)
	{
		...
		LLOGD("Lease time: %u seconds", w5500->dhcp_client.lease_time);
		int i;
		for(i = 0; i < MAX_DNS_SERVER; i++)
		{
...
static void w5500_link_state(w5500_ctrl_t *w5500, uint8_t check_state)
{
	...
	if (w5500->link_ready != check_state)
	{
		...
		w5500_nw_state_cb(w5500->link_ready ? W5500_CABLE_INSERT : W5500_CABLE_REMOVE, 0);
		if (w5500->link_ready)
		{
			if (!w5500->static_ip)
			{
...
static void w5500_init_reg(w5500_ctrl_t *w5500)
{
	...
	if (w5500->static_ip)
	{
		BytesPutLe32(&temp[W5500_COMMON_GAR0], w5500->static_gateway);	//It is already in big-endian format and does not need to be converted.
		BytesPutLe32(&temp[W5500_COMMON_SUBR0], w5500->static_submask); //It is already in big-endian format and does not need to be converted.
		BytesPutLe32(&temp[W5500_COMMON_IP0], w5500->static_ip); //It is already in big-endian format and does not need to be converted.
	}
...
void w5500_init(luat_spi_t* spi, uint8_t irq_pin, uint8_t rst_pin, uint8_t link_pin)
{
	...
	if (!prv_w5500_ctrl)
	{
		...
		LLOGD("w5500 task ret %d", ret); // TODO handles startup failure
