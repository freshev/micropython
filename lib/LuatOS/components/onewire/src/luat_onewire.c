...
static const luat_onewire_opt_t* get_opt(luat_onewire_ctx_t* ctx) {
    ...
    if (ctx->mode != LUAT_ONEWIRE_MODE_GPIO) {
        if (opt_hw == NULL) {
            LLOGE("The current hardware does not support hardware mode driver Onewire");
            return NULL;
        }
        opt = opt_hw; 
    }
...
//TODO GPIO default implementation, can be migrated from lib_sensor
...
int luat_onewire_ds18b20(luat_onewire_ctx_t* ctx, int check_crc, int32_t *re) {
    ...
    //Initialize single bus
    luat_os_entry_cri();
    int ret = luat_onewire_setup(ctx);
    if (ret)
    {
        LLOGW("setup failed mode %d id %d ret %d", ctx->mode, ctx->id, ret);
        goto exit;
    }
    //reset device
    ret = ds18b20_reset_gpio(ctx);
    // if (ret) {
    //     LLOGW("reset failed mode %d id %d ret %d", ctx->mode, ctx->id, ret);
    //     return 0;
    // }
    ...
    if (ret)
    {
        LLOGW("Connect failed mode %d id %d ret %d", ctx->mode, ctx->id, ret);
        goto exit;
    }
    ...
    if (ret)
    {
        LLOGW("Connect failed 2 mode %d id %d ret %d", ctx->mode, ctx->id, ret);
        goto exit;
    }
    ...
    uint8_t data[9] = {0};
    // Read 9 bytes in check mode, otherwise read 2 bytes
    ret = luat_onewire_read(ctx, (char*)data, check_crc ? 9 : 2);
    luat_onewire_close(ctx); // No subsequent reading is required
    if (ret != 9)
    {
        LLOGW("Read failed 2 mode %d id %d ret %d", ctx->mode, ctx->id, ret);
        goto exit;
    }
    ...
    luat_onewire_close(ctx); // Clean up and close
    luat_os_exit_cri();
    *re = val;
    return 0;
exit:
...
int luat_onewire_dht(luat_onewire_ctx_t* ctx, int32_t* temp, int32_t* hm, int check_crc) {
    ...
    dht_reset(ctx->id);
    luat_os_entry_cri();
    if(dht_connect(ctx->id))//Not connected
    {
        luat_os_exit_cri();
        return -1;
    }
    ...
    // LLOGD("Read data %02X%02X%02X%02X%02X", buff[0], buff[1], buff[2], buff[3], buff[4]);

    if (check_crc) {
        uint8_t check_r = 0;
        check_r = buff[0]+buff[1]+buff[2]+buff[3];
        if (check_r != buff[4]) {
            // LLOGD("crc verification failed");
            return -2;
        }
        // LLOGD("crc verification successful");
    }
