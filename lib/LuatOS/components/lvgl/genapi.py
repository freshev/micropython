...
#Various names, but all have int variants.
        ...
        try :
            ...
            if node.type.__class__.__name__ == "PtrDecl":
                ...
                if type(node.type.type) == c_ast.TypeDecl :
                    ...
                    #The return value is char**, which is not supported yet.
            ...
            if not method_name.startswith(self.prefix):
                return
            #Some callback methods, these cannot be automatically generated
            ...
            if method_name in ["lv_btnmatrix_set_map", "lv_calendar_set_month_names",
                ...
                return
            #Because points[] cannot handle the method
            if method_name in ["lv_indev_set_button_points", "lv_draw_triangle",
                               ...
                               "lv_draw_polygon"] :
                return
            #Because various arrays cannot be processed
            if method_name in ["lv_btnmatrix_set_ctrl_map", "lv_keyboard_set_ctrl_map", "lv_calendar_set_highlighted_dates",
                ...
                return
            #It is unlikely that anyone will use this method. The return value is uint8_t*, which is rare.
            ...
            for prefix in methods[group] :
                ...
                with open("gen/"+group+"/luat_" + prefix + ".c", "w") as f :
                    ...
                    for m in methods[group][prefix] :
                        ...
                        #Processing method return value
                        gen_lua_ret(m["ret"], f)
...
map_lua_arg = {
    ...
    "lv_anim_enable_t" : {"fmt": "{} {} = (lv_anim_enable_t)lua_toboolean(L, {});", "incr" : 1},#Equivalent to uint8
    "lv_scrollbar_mode_t" :  {"fmt": "{} {} = (lv_scrollbar_mode_t)luaL_checkinteger(L, {});", "incr" : 1},#Equivalent to uint8
    "lv_layout_t" : {"fmt": "{} {} = (lv_layout_t)luaL_checkinteger(L, {});", "incr" : 1},#Equivalent to uint8
...
def gen_lua_ret(tp, f) :
    ...
    #A good match
        ...
        for line in map_lua_ret[tp] :
            ...
            f.write(line)
            f.write("\n")
    #lv_color_t needs special processing
    elif tp == "lv_color_t" :
