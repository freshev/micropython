...
/*@Modules mlx90640
@summary Infrared temperature measurement (MLX90640)
@version 1.0
@date 2022.1.20
...
@api mlx90640.init(i2c_id,refresh_rate) (Note: To use this interface after 2023.5.15, users need to initialize the i2c interface by themselves)
@int The i2c bus ID or soft i2c object where the sensor is located, the default is 0
@int The measurement rate of the sensor, the default is 4Hz
...
if mlx90640.init(0,mlx90640.FPS4HZ) then
    ...
    while 1 do
        mlx90640.feed() -- Get a frame of data
        mlx90640.draw2lcd(0, 0,1)-- LCD needs to be initialized in advance
...
static int l_mlx90640_init(lua_State *L){
    ...
    //After initialization, read the frame here first and remove some error data after initialization.
...
/*Get the underlying raw data, floating point matrix
@api mlx90640.raw_data()
@return table floating point data, temperature values   corresponding to 768 pixels*/
...
@return number Single point temperature value*/
...
@return number case temperature*/
...
@return number Highest temperature position*/
...
@return number lowest temperature position*/
...
@int Magnification factor, must be an exponential multiple of 2 (1, 2, 4, 8, 16...) The default is 1
