...
static int sfd_w25q_init (void* userdata) {
    ...
    uint8_t cmd = 0x9F;
    //Send CMD 9F and read capacity information
    ...
    if (buff[1] == 0x40) {
        switch(buff[2]) {
            ...
            case 0x19:
                ...
                break;
            default :
                drv->sector_count = 16*256;// Default is 16M
                break;
        }
    }
    else {
...
static int sfd_w25q_status (void* userdata) {
    sfd_drv_t *drv = (sfd_drv_t *)userdata;
    return drv->sector_count == 0 ? 0 : 1; // TODO returns according to BUSY status
}
