...
// TODO should be connected to vfs, not directly to lfs
...
int luat_fskv_size(const char* key, char buff[4]) {
    ...
    if (ret > 1 && ret < 256) {
        ...
        if (ret2 != ret) {
            ret = -2; // Failed to read, there must be something wrong
        }
    }
    lfs_file_close(&sfd_lfs->lfs, &fd);
    return ret;
...
int luat_fskv_next(char* buff, size_t offset) {
    lfs_dir_t dir = {0};
    struct lfs_info info = {0};
    // The offset needs to be +2, because the first two values     are "." and ".." dirs
    offset += 2;
    int ret = lfs_dir_open(&sfd_lfs->lfs, &dir, "");
    if (ret < 0) {
        LLOGW("lfs_dir_open ret %d", ret);
