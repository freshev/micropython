...
int luat_lcd_draw(luat_lcd_conf_t* conf, int16_t x1, int16_t y1, int16_t x2, int16_t y2, luat_color_t* color) {
    ...
    //Direct swipe mode
    if (conf->buff == NULL) {
        // Regular data, overall transmission
        ...
        // Data beyond the boundary is transmitted row by row
        else {
            ...
            // TODO
            // LLOGD("Out of bounds, special handling");
        }
        return 0;
    }
    // buff mode
...
int luat_lcd_draw_point(luat_lcd_conf_t* conf, int16_t x, int16_t y, luat_color_t color) {
    ...
    if (conf->port != LUAT_LCD_HW_ID_0)
        tmp = color_swap(color);// Note that the color needs to be swapped here.
...
int luat_lcd_draw_line(luat_lcd_conf_t* conf,int16_t x1, int16_t y1, int16_t x2, int16_t y2,luat_color_t color) {
    ...
    if (x1 == x2 || y1 == y2) // straight line
    {
        ...
        if (conf->port != LUAT_LCD_HW_ID_0)
            tmp = color_swap(color);// color swap
        if (line_buf) {
            for (i = 0; i < dots; i++)
            {
                line_buf[i] = tmp;
