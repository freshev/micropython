...
/*@Modules max30102
@summary Heart rate Modules (MAX30102)
@version 1.0
@date 2023.2.28
...
@int i2c bus id where the sensor is located, default is 0
@int int pin
...
/*Get heart rate and blood oxygen (it takes about 10 seconds to measure)
@api max30102.get()
@return bool returns true if successful, otherwise returns nil or false
@return number heart rate
@return number blood oxygen*/
static int l_max30102_get(lua_State *L) {
    if (max30102_idp){
        ...
        if (ret) {
            LLOGE("max30102 Thread startup failed %d", ret);
            luat_pushcwait_error(L,1);
        }
    }
    return 1;
