...
-- This Modules is implemented by Ant Chain and is open to users.
-- Please check the demo for specific usage, and contact Ant Chain for technical support.*/
#include "luat_base.h"
#include "rotable2.h"
...
/*initialization
@apiantbot.init()
@return int 0: success, other values   are failure
@usage

-- Initialize the underlying adaptation of Ant Chain
antbot.init()*/
...
@return string version number, if the acquisition fails, return nil*/
...
static luat_bot_data_pac(lua_State *L)
{
    ...
    uint8_t *data_out = luat_heap_malloc(outlen); // TODO changed to lua_Buff
    if (!data_out) {
