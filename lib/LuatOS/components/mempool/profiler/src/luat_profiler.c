...
void* luat_profiler_alloc(void *ud, void *ptr, size_t osize, size_t nsize) {
    // TODO add debugging information to the head so that the actual malloc and realloc sizes can be recorded
    void* dst = NULL;
    if (ctx.tag == 0) { //Disabled state, return as is
    ...
    // If the pointer is not NULL and the target size is 0, it is free
    ...
    // If the pointer is NULL and the target size is not 0, it is malloc
    else if (ptr == NULL && nsize > 0) {
        ctx.counter_malloc ++;
...
void luat_profiler_print(void) {
    ...
    //Memory size before and after output
