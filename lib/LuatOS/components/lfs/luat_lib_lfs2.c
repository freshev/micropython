...
/**
Mount lifftefs, currently supports spi flash and memory.
@api lfs2.mount(path, drv, formatOnFail)
@string mount path
@userdata Additional data required for mounting, currently only supports sfd
@bool Whether to try to format when mounting fails, the default is false
@usage
...
{ 
  { "mount",	ROREG_FUNC(l_lfs2_mount)}, //Initialize, mount
// { "unmount", ROREG_FUNC(l_lfs2_unmount)}, // Unmount
  { "mkfs",		ROREG_FUNC(l_lfs2_mkfs)}, // Format!!!
  { NULL,		  ROREG_INT(0)}
};
