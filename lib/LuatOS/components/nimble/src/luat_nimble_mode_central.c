...
typedef struct luat_nimble_scan_result
{
    ...
    char name[64];
    char addr[7]; //Address type + MAC address
...
int luat_nimble_central_disc_srv(int id) {
    ...
    if (!g_ble_conn_handle) {
        LLOGW("No connection established yet");
        return -1;
    }
...
int luat_nimble_scan_cb(lua_State*L, void*ptr) {
    ...
    if (res->name[0]) {
        ...
        lua_pushliteral(L, ""); //The device has no name
    }
    lua_newtable(L);
    // char buff[64];
    ...
    if (error->status == 0) {
        ...
        for (size_t i = 0; i < MAX_PER_SERV * MAX_PER_SERV; i++)
        {
            ...
            if (0 == ble_uuid_cmp(uuid_ccc, &dsc->uuid)) {
                LLOGD("Set chr %d dsc handle %d", peer_chrs[i]->val_handle, dsc->handle);
...
static int chr_disced(uint16_t conn_handle,
    ...
    for (size_t i = 0; i < MAX_PER_SERV; i++)
    {
        ...
        if (peer_servs[i] == arg) {
            ...
            // LLOGD("Characteristic value %s flags %d", ble_uuid_to_str(&chr->uuid, buff), chr->properties);
