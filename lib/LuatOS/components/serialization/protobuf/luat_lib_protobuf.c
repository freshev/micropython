...
@return int How long was read, for debugging
@usage
-- Note that the same file only needs to be loaded once, unless protobuf.clear() is called
protobuf.load(io.readFile("/luadb/person.pb"))*/
...
@api protobuf.clear()
@return nil no return value, always successful
@usage
...
/*Encoding protobuffs packets
@api protobuf.encode(tpname, data)
@string data type name, defined in pb file, loaded by protobuf.load
@table The data to be encoded must be a table, and the content conforms to the definition in the pb file
@return string encoded data, nil will be returned if failed
@usage
-- Data encoding
local tb = {
    name = "wendal"
...
if pbdata then
    -- Note that encoded data usually contains invisible characters
...
#define ENTRY(name) { #name, ROREG_FUNC(Lpb_##name) }
        ENTRY(clear),
        ENTRY(load),
        // ENTRY(loadfile), // loadfile is not supported yet, please transfer it through io.readFile
        ENTRY(encode),
        ENTRY(decode),
        // ENTRY(types),
        // ENTRY(fields),
