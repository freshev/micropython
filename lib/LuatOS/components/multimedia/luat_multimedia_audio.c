...
//The general methods of the above functions need to be implemented
...
//General method to be implemented
...
LUAT_WEAK int luat_audio_init(uint8_t multimedia_id, uint16_t init_vol, uint16_t init_mic_vol){
    ...
    if (audio_conf->bus_type == LUAT_AUDIO_BUS_I2S){
        ...
        if (!audio_conf->pa_is_control_enable)	//The state that PA cannot control is controlled by muting.
        {
        ...
        {
            luat_audio_pm_request(multimedia_id,LUAT_AUDIO_PM_STANDBY); //Enter standby mode by default
...
LUAT_WEAK int luat_audio_pm_request(uint8_t multimedia_id,luat_audio_pm_mode_t mode){
    ...
    if (audio_conf!=NULL && audio_conf->bus_type == LUAT_AUDIO_BUS_I2S){
    	...
    	if (audio_conf->codec_conf.codec_opts->no_control)	//codec has no register
    	{
			switch(mode)
			{
			case LUAT_AUDIO_PM_RESUME:
				...
				break;
			case LUAT_AUDIO_PM_STANDBY:	//Only turn off PA, not codec, so you don't need to restart codec the next time you play, saving startup time.
				luat_audio_power_keep_ctrl_by_bsp(1);
				luat_audio_play_blank(multimedia_id, 1);
				luat_audio_pa(multimedia_id,0,0);
				break;
    	...
    	else	//When codec has registers
    	{
			...
			case LUAT_AUDIO_PM_RESUME:
				...
				if (LUAT_AUDIO_PM_POWER_OFF == audio_conf->sleep_mode)	//The power has been forced off before, so it must be re-initialized.
				{
