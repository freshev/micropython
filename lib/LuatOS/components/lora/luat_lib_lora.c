...
sys.subscribe("LORA_TX_DONE", function()
    ...
    case LORA_RX_DONE:
/*@sys_pub lora
LORA reception completed
LORA_RX_DONE
@usage
sys.subscribe("LORA_RX_DONE", function(data, size, rssi, snr)
    -- rssi and snr added on 2023-09-06
...
@table lora configuration parameters, related to specific equipment
@usage
lora.init("llcc68",
    {
        id = 0, -- SPI id
        cs = pin.PB04, -- GPIO number of SPI chip select. If there is no pin library, just fill in the GPIO digital number.
        res = pin.PB00, -- the GPIO number connected to the reset pin. If there is no pin library, just fill in the GPIO digital number.
        busy = pin.PB01, -- GPIO number of busy detection pin
...
/*lora configuration sending parameters
@api lora.set_txconfig(ic, txconfig)
@string lora model, currently supported:<br>llcc68<br>sx1268
@table lora sends configuration parameters, related to specific devices
@usage
...
/*lora configuration receiving parameters
@api lora.set_rxconfig(ic, set_rxconfig)
@string lora model, currently supported:<br>llcc68<br>sx1268
@table lora receives configuration parameters, which are related to specific devices
@usage
...
-- The old version does not have recv, it can be changed to lora.recive
lora.recv(1000)*/
...
/*Set entry mode (sleep, normal, etc.)
@api lora.mode(mode)
@number mode normal mode: lora.STANDBY sleep mode: lora.SLEEP default is normal mode
@usage
