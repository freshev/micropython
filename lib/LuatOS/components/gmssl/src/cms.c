...
{
	size_t len = content_len; // Note: Since header_to_der does not output data, the length of the data needs to be added, and header length is the correct value.
	/*
...
// There is obviously a problem with this function. The caller does not know how big a buffer should be prepared.
// The maximum buffer value that should be output to a maxlen is content_len.
int cms_enced_content_info_decrypt_from_der(
	...
	const uint8_t **shared_info1, size_t *shared_info1_len,// Support optional null output
	const uint8_t **shared_info2, size_t *shared_info2_len,// Support optional null output
	const uint8_t **in, size_t *inlen)
{
	SM4_KEY sm4_key;
	const uint8_t *iv;
	...
	uint8_t **out, size_t *outlen)
{
	...
	// When content_type == OID_cms_data, data is raw data, which is encapsulated as OCTET STRING encoded output
	// When content_type is other types, data is the DER data of TLV
	// It has been processed in to_der/from_der, but it also needs to be processed when calculating the hash value.
	p = content_header;
...
int cms_enveloped_data_encrypt_to_der(
	...
	uint8_t **out, size_t *outlen)
{
	uint8_t rcpt_infos[1024]; //How big does it need to be?
