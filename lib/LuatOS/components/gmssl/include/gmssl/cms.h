...
int cms_content_info_from_der(
	int *content_type,
	const uint8_t **content, size_t *content_len, // What is obtained here is the complete TLV
...
int cms_signed_data_sign_to_der(
	...
	int content_type, const uint8_t *data, size_t datalen, // When OID_cms_data is raw data
	const uint8_t *crls, size_t crls_len, // can be empty
	uint8_t **out, size_t *outlen);
int cms_signed_data_verify_from_der(
	...
	int *content_type, const uint8_t **content, size_t *content_len, // Should raw data be returned?
	const uint8_t **certs, size_t *certs_len,
	const uint8_t **crls, size_t *crls_len,
	const uint8_t **signer_infos, size_t *signer_infos_len,
	const uint8_t **in, size_t *inlen);
...
Since the type of encryptedKey is SM2Cipher, and there are 2 INTEGERs in SM2Cipher, the length is not fixed.
Therefore the output length cannot be predetermined*/
...
int cms_recipient_info_from_der(
	...
	const uint8_t **issuer, size_t *issuer_len,
	const uint8_t **serial_number, size_t *serial_number_len,
	int *pke_algor, const uint8_t **params, size_t *params_len,// SM2 encryption only uses SM3 and has no default parameters, but ECIES may have
	const uint8_t **enced_key, size_t *enced_key_len,
	const uint8_t **in, size_t *inlen);
...
//The following is the public API
// Design considerations for public API:
// 1. No need to call other functions
// 2. Logically easy to understand
// 3. Treat cms and cmslen as objects
...
int cms_encrypt(
	uint8_t *cms, size_t *cmslen, // Output ContentInfo (type encryptedData)
	int enc_algor, const uint8_t *key, size_t keylen, const uint8_t *iv, size_t ivlen, // Symmetric encryption algorithm, key and IV
	int content_type, const uint8_t *content, size_t content_len, //Input data to be encrypted
	const uint8_t *shared_info1, size_t shared_info1_len, // Additional Information
	const uint8_t *shared_info2, size_t shared_info2_len);

int cms_decrypt(
	const uint8_t *cms, size_t cmslen, //Input ContentInfo (type encryptedData)
	int *enc_algor, const uint8_t *key, size_t keylen, // Decryption key (we don't know the decryption algorithm)
	int *content_type, uint8_t *content, size_t *content_len, //Output decrypted data type and data
	const uint8_t **shared_info1, size_t *shared_info1_len, // Additional Information
	const uint8_t **shared_info2, size_t *shared_info2_len);

int cms_sign(
	uint8_t *cms, size_t *cms_len,
	const CMS_CERTS_AND_KEY *signers, size_t signers_cnt, // Signer's signing private key and certificate
	int content_type, const uint8_t *content, size_t content_len, //Input data to be signed
	const uint8_t *crls, size_t crls_len);
...
int cms_envelop(
	uint8_t *cms, size_t *cms_len,
	const uint8_t *rcpt_certs, size_t rcpt_certs_len, // Recipient certificate, note that the type of this parameter can accommodate multiple certificates, but it is most convenient for the caller to only have one recipient.
	int enc_algor, const uint8_t *key, size_t keylen, const uint8_t *iv, size_t ivlen, // Symmetric encryption algorithm and parameters
	int content_type, const uint8_t *content, size_t content_len, //Input data to be encrypted
	const uint8_t *shared_info1, size_t shared_info1_len, //Additional input information
	const uint8_t *shared_info2, size_t shared_info2_len);

int cms_deenvelop(
	const uint8_t *cms, size_t cms_len,
	const SM2_KEY *rcpt_key, const uint8_t *rcpt_cert, size_t rcpt_cert_len, // The decryption private key and corresponding certificate of the receiver. Note that only one decryption party is needed.
	int *content_type, uint8_t *content, size_t *content_len,
	const uint8_t **rcpt_infos, size_t *rcpt_infos_len, // Parsed and used for display
	const uint8_t **shared_info1, size_t *shared_info1_len,
	const uint8_t **shared_info2, size_t *shared_info2_len);
...
// Generate ContentInfo, type == keyAgreementInfo
