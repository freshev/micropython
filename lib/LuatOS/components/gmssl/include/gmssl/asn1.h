...
int asn1_any_to_der(const uint8_t *a, size_t alen, uint8_t **out, size_t *outlen); // The caller should ensure that a,alen is TLV
int asn1_any_from_der(const uint8_t **a, size_t *alen, const uint8_t **in, size_t *inlen); // This function checks whether the input is TLV
...
// asn1_integer_ does not support negative number encoding and decoding
...
// asn1_int_ only supports encoding and decoding of small unsigned integers and does not support negative numbers.
int asn1_int_to_der_ex(int tag, int val, uint8_t **out, size_t *outlen); // When val == -1, no output, return 0
int asn1_int_from_der_ex(int tag, int *val, const uint8_t **in, size_t *inlen); // Negative numbers are not supported, *val is set to -1 when returning 0
...
//The bit length does not have to be an integer multiple of 8
...
// The bit length must be an integer multiple of 8, so use the byte length
...
// bits == -1 does not encode, only supports a smaller number of bits
...
// names[i] corresponds to the i-th bit
...
// If a correctly parsed OID is not in the infos list, then 1 is still returned, but the caller must check whether the returned info is empty
...
// d,dlen are the values   in SEQUENCE OF, SET OF
