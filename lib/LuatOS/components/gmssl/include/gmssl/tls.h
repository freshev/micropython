...
typedef enum {
	...
	TLS_cipher_ecdhe_sm4_cbc_sm3		= 0xe011, // You can make TLSv1.2 use this
...
// Instead of supporting v2, it is better to modify v2 directly to make v2 and v3 compatible
...
// The parsing of handshake messages ServerKeyExchange and ClientKeyExchange depends on the current cipher suite
...
// This function is special. It directly parses the certificate chain instead of returning a pointer.
// A separate parsing function should be provided to parse the TLS certificate chain
...
int tls_record_set_handshake_client_key_exchange_ecdhe(uint8_t *record, size_t *recordlen,
	const SM2_POINT *point); // SM2_POINT type should not be supported here
...
// Finished
// FIXME: Support TLS 1.3, provide MIN, MAX or TLS12, TLS13, TLCP...
#define TLS_VERIFY_DATA_SIZE 12 //TLS 1.3 or other versions support longer verify_data
...
typedef struct {
	...
	// Actually this is not quite right, there should still be a complete ciphertext record
	...
	uint8_t server_certs[TLS_MAX_CERTIFICATES_SIZE]; // Dynamic might be better
...
#define TLS_MAX_EXTENSIONS_SIZE 512 // This value should be reconsidered. Can it be loaded with other buffers?
