/***************
	demo_hello
****************/
#include <string.h>
#include "iot_debug.h"
#include "iot_os.h"

#include "am_openat_drv.h"

#include "iot_vat.h"

#include "iot_keypad.h"
#include "am_openat.h"

#define SW_BYA 18
#define SW_BYB 19   

typedef struct
{
    BOOL bPressed;
    UINT8 swkey;
} POC_SWKEY_MESSAGE;

HANDLE poc_lcd_test_hand = NULL;

#define LCD_WIDTH 162
#define LCD_HEIGH 132
#define LCD_PIXEL_BYTES 2 //Two bytes 16 colors

char lcd_buf[LCD_WIDTH * LCD_HEIGH * LCD_PIXEL_BYTES] = {0};

#define WHITE 0xFFFF   //White
#define BLACK 0x0000   //black
#define BLUE 0x001F    //blue
#define BRED 0X8802    //Black and red
#define GRED 0XFBE4    //Green and red
#define GBLUE 0X9EDD   //Blue and green
#define RED 0xF800     //red
#define MAGENTA 0xF81F //Yanghong
#define GREEN 0x07E0   //green
#define CYAN 0x051D    //blue
#define YELLOW 0xFFE0  //yellow
#define BROWN 0XBC40   //brown
#define BRRED 0XFC07   //Brown and red
#define GRAY 0X8430    //grey

u16 disp_bkcolor[] = {WHITE, BLACK, BLUE, BRED, GRED, GBLUE, RED, MAGENTA, GREEN, CYAN, YELLOW, BROWN, BRRED, GRAY};

const UINT32 lcdRegTable[] =
    {
        0x00020011,
        0x00010078,
        0x000200B1,
        0x00030005,
        0x0003003a,
        0x0003003a,
        0x000200B2,
        0x00030005,
        0x0003003a,
        0x0003003a,
        0x000200B3,
        0x00030005,
        0x0003003a,
        0x0003003a,
        0x00030005,
        0x0003003a,
        0x0003003a,
        0x000200B4,
        0x00030003,
        0x000200C0,
        0x00030062,
        0x00030002,
        0x00030004,
        0x000200C1,
        0x000300C0,
        0x000200C2,
        0x0003000D,
        0x00030000,
        0x000200C3,
        0x0003008D,
        0x0003006A,
        0x000200C4,
        0x0003008D,
        0x000300EE,
        0x000200C5,
        0x00030012,
        0x000200E0,
        0x00030003,
        0x0003001B,
        0x00030012,
        0x00030011,
        0x0003003F,
        0x0003003A,
        0x00030032,
        0x00030034,
        0x0003002F,
        0x0003002B,
        0x00030030,
        0x0003003A,
        0x00030000,
        0x00030001,
        0x00030002,
        0x00030005,
        0x000200E1,
        0x00030003,
        0x0003001B,
        0x00030012,
        0x00030011,
        0x00030032,
        0x0003002F,
        0x0003002A,
        0x0003002F,
        0x0003002E,
        0x0003002C,
        0x00030035,
        0x0003003F,
        0x00030000,
        0x00030000,
        0x00030001,
        0x00030005,
        0x000200FC,
        0x0003008c,
        0x0002003A,
        0x00030005,
        0x00020036,
        0x00030060,
        0x00020029,
};

static void write_command_table(const UINT32 *table, UINT16 size)
{
    UINT16 flag;
    UINT16 value;
    UINT16 index;

    for (index = 0; index < size && table[index] != (UINT32)-1; index++)
    {
        flag = table[index] >> 16;
        value = table[index] & 0xffff;

        switch (flag)
        {
        case 1:
            iot_os_sleep(value);
            break;

        case 0:
        case 2:
            iot_lcd_write_cmd(value & 0x00ff);
            break;

        case 3:
            iot_lcd_write_data(value & 0x00ff);
            break;

        default:
            break;
        }
    }
}

BOOL lcdInit(void)
{
    T_AMOPENAT_COLOR_LCD_PARAM param;
    iot_pmd_poweron_ldo(OPENAT_LDO_POWER_VLCD, 15);
    memset(&param, 0, sizeof(param));
    param.width = LCD_WIDTH;
    param.height = LCD_HEIGH;
    param.msgCallback = NULL;
    param.bus = OPENAT_LCD_SPI4LINE;
    iot_lcd_color_init(&param);
    write_command_table(lcdRegTable, sizeof(lcdRegTable) / sizeof(int));
    return TRUE;
}

void poc_lcd_test()
{
    OPENAT_gpioPulse(13, 1, 6000, 1, 0); //BCT3220 backlight control chip reset
    OPENAT_gpioPulse(13, 40, 10, 1, 1);  //BCT3220 backlight controls a falling edge, 100% brightness

    lcdInit();
    T_AMOPENAT_LCD_RECT_T rect = {0};
    rect.ltX = 0;
    rect.ltY = 0;
    rect.rbX = LCD_WIDTH;
    rect.rbY = LCD_HEIGH;

    uint8 color = 0;
    int msgId;
    POC_SWKEY_MESSAGE *mymsg = NULL;
    while (1)
    {
        OPENAT_wait_message(poc_lcd_test_hand, &msgId, &mymsg, OPENAT_OS_SUSPENDED);
        if (mymsg->swkey == SW_BYA)
        {
            OPENAT_wait_message(poc_lcd_test_hand, &msgId, &mymsg, 100);
            if (mymsg->swkey == SW_BYB)
            {
                iot_debug_print("[knob_lcd] The knob rotates clockwise");
                color++;
            }
        }
        else if (mymsg->swkey == SW_BYB)
        {
            OPENAT_wait_message(poc_lcd_test_hand, &msgId, &mymsg, 100);
            if (mymsg->swkey == SW_BYA)
            {
                iot_debug_print("[knob_lcd] The knob rotates counterclockwise");
                color--;
            }
        }

        u16 *pPixel16 = (u16 *)lcd_buf;
        for (u16 col = 0; col < LCD_WIDTH; col++)
        {
            for (u16 row = 0; row < LCD_HEIGH; row++)
            {
                pPixel16[row * LCD_WIDTH + col] = disp_bkcolor[color % (sizeof(disp_bkcolor) / sizeof(disp_bkcolor[0]))];
            }
        }
        iot_lcd_update_color_screen(&rect, lcd_buf);
    }
}

void gpio_handle(E_OPENAT_DRV_EVT evt, E_AMOPENAT_GPIO_PORT gpioPort, unsigned char state)
{
    //Judge it as a gpio interrupt
    if (evt == OPENAT_DRV_EVT_GPIO_INT_IND)
    {
        iot_debug_print("[knob_lcd] input handle gpio %d, state %d", gpioPort, state);
        static POC_SWKEY_MESSAGE msg = {0};
        msg.bPressed = state;
        msg.swkey = gpioPort;
        OPENAT_send_message(poc_lcd_test_hand, 0, &msg, 0);
    }
}

int appimg_enter(void *param)
{
    iot_debug_print("[knob_lcd]appimg_enter");
    // Turn off the watchdog, the crash will not restart. Open by default
    iot_debug_set_fault_mode(OPENAT_FAULT_HANG);
    iot_os_sleep(100);
    //Set keypad, line 3 and column 2 are physically forced download keys (OK keys). Pressing this button when powering on will enter download mode
    //The poc project does not Introduction the boot key. You must call the keypad once to enter the download mode, otherwise you can only dismantle the machine after turning into bricks.
    iot_vat_send_cmd("AT*DOWNLOAD=2,3,2\r\n", sizeof("AT*DOWNLOAD=2,3,2\r\n"));
    iot_os_sleep(100);
    //Open debug information, close by default
    iot_vat_send_cmd("AT^TRACECTRL=0,1,2\r\n", sizeof("AT^TRACECTRL=0,1,2\r\n"));
    iot_os_sleep(100);

    poc_lcd_test_hand = iot_os_create_task(poc_lcd_test, NULL, 1024, 1, OPENAT_OS_CREATE_DEFAULT, "hello");

    T_AMOPENAT_GPIO_CFG input_cfg = {0};
    input_cfg.mode = OPENAT_GPIO_INPUT_INT;                     //Configuration input interrupt
    input_cfg.param.intCfg.debounce = 10;                       //Anti-shake 10ms
    input_cfg.param.intCfg.intType = OPENAT_GPIO_INT_BOTH_EDGE; //Binding trigger method double edge
    input_cfg.param.intCfg.intCb = gpio_handle;                 //Interrupt handling function
    input_cfg.param.pullState = OPENAT_GPIO_NO_PULL;            //High resistance state
    iot_gpio_open(SW_BYA, &input_cfg);                          //Initialize gpio to high-impedance interrupt input mode
    iot_gpio_open(SW_BYB, &input_cfg);                          //Initialize gpio to high-impedance interrupt input mode

    return 0;
}

void appimg_exit(void)
{
    iot_debug_print("[knob_lcd] appimg_exit");
}
