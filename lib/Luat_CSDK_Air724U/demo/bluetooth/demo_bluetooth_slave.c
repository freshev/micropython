...
/*Ïèìn¼ó · þîñ £ ¬ôúòà´îìn¼óìøõ ÷ £ ¬èôìøõ ÷ ïâ ° üº¬ìøðôãèêö £ ¬ôòìn¼óìøõ ÷ ºó½Ó × Åìn¼óÃÈÊÖ*/
BOOL AddService()
{
    int i,j;
    ...
    uuid.uuid_short = BT_8910_TP_UUID;// Þîñuid
    uuid.uuid_type = UUID_SHORT;
    uuid_c1.uuid.uuid_short = BT_8910_TP_UUID_CHAR;//ÌØÕ÷uuid
    uuid_c1.uuid.uuid_type = UUID_SHORT;
    uuid_c1.attvalue = ATT_CHARA_PROP_READ | ATT_CHARA_PROP_WRITE;//ÌØÕ÷ÊôÐÔ
    uuid_c1.permisssion = ATT_PM_READABLE|ATT_PM_WRITEABLE;//ÌØÕ÷È¨ÏÞ
    uuid_c2.uuid.uuid_short = BT_8910_FEEDBACK_CHAR;//ÌØÕ÷uuid
    uuid_c2.uuid.uuid_type = UUID_SHORT;
    uuid_c2.attvalue = ATT_CHARA_PROP_READ | ATT_CHARA_PROP_NOTIFY | ATT_CHARA_PROP_INDICATE;//ÌØÕ÷ÊôÐÔ
    uuid_c2.permisssion = ATT_PM_READABLE;//ÌØÕ÷È¨ÏÞ
    ...
    memcpy(bt_descriptor[0].value,"123456789",strlen("123456789"));// ãèêöêôðô
    bt_descriptor[1].uuid.uuid_short = ATT_UUID_CLIENT;// Ãèêöuuid
    bt_descriptor[1].uuid.uuid_type = UUID_SHORT;
    bt_descriptor[1].configurationBits = 1;// ãèêöêôðô
    ...
    iot_ble_iotctl(0,BLE_ADD_SERVICE,param1);// ìn¼ó · Þîñ
    if(param1.uuid != NULL)
    ...
    for(i = 0;i < sizeof(service_param)/sizeof(ble_add_characteristic_t);i ++)
    {
        ...
        iot_ble_iotctl(0,BLE_ADD_CHARACTERISTIC,param2);//Ìn¼ÓÌØÕ÷
        ...
        if(service_param[i].count != 0)
        {
            for(j = 0;j < service_param[i].count;j ++)
            {
                ...
                iot_ble_iotctl(0,BLE_ADD_DESCRIPTOR,param3);//Ìn¼ÓÃèÊö
...
BOOL advertising(VOID)
{
    ...
    //UINT8 data1[BLE_MAX_ADV_MUBER] = {0x02,0x01,0x06,0x04,0xff,0x01,0x02,0x03};//¹ã²¥°üÊý¾Ý
    //UINT8 data2[BLE_MAX_ADV_MUBER] = {0x02,0x0a,0x04};//ÏìÓ¦°üÊý¾Ý
    ...
    //T_OPENAT_BLE_ADV_PARAM advparam = {0x80,0xa0,0,0,0,"11:22:33:44:55:66",0x07,0};//¹ã²¥²ÎÊý
...
/*param2.advdata = iot_os_malloc(sizeof(T_OPENAT_BLE_ADV_DATA));
    ...
    iot_ble_iotctl(0,BLE_SET_ADV_DATA,param2);//ÉèÖÃ¹ã²¥°üÊý¾Ý
...
BOOL ble_data_trans(VOID)
{
    ...
    while(1)
    {
        ...
        iot_os_wait_message(ble_test_handle,&msg);//µÈ´ý½ÓÊÕµ½Êý¾Ý
...
VOID ble_test(VOID)
{
    ...
    //1.  ´ò¿ªÀ¶ÑÀ
    ble_poweron();
    iot_os_wait_message(ble_test_handle,&msg);//µÈ´ýÀ¶ÑÀ´ò¿ª
    if(msg->eventid == OPENAT_BT_ME_ON_CNF)
    {
        ...
        //2.¹ã²¥À¶ÑÀ
        advertising();
        iot_os_wait_message(ble_test_handle,&msg);// µè´ýe¬~é³é¹¦
