...
void bluetooth_callback(T_OPENAT_BLE_EVENT_PARAM *result)
{
	...
	switch(result->id)
	{
		case OPENAT_BLE_RECV_DATA:
			...
			{
                msg->len  =  result->len;
                msg->uuid_flag  =  result->uuid_flag;//1:128λuuid 0:16λuuid
                msg->uuid  =  result->uuid;//16λuuid
                memcpy(msg->long_uuid, result->long_uuid, sizeof(result->long_uuid));//128λuuid
        ...
        case OPENAT_BLE_SET_SCAN_REPORT:
            ...
            iot_debug_print("[bluetooth]bt scan rssi %d",(int8)ble_scanInfo->rssi);// ðåºåç¿¶è
            ...
            iot_debug_print("[bluetooth]bt scan manu_data %s",manu_data);//³§ÉÌÊý¾Ý
            AppConvertBinToHex(ble_scanInfo->raw_data,ble_scanInfo->data_length,raw_data);
            raw_data[ble_scanInfo->data_length*2] = '\0';
            iot_debug_print("[bluetooth]bt scan raw_data %s",raw_data);//¹ã²¥Ô­Ê¼Êý¾Ý
        ...
        case OPENAT_BLE_FIND_CHARACTERISTIC_UUID_IND:
            if(result->uuid_flag == UUID_SHORT)//1:128λuuid 0:16λuuid
            {
...
BOOL scan(VOID)
{
    ...
    //T_OPENAT_BLE_SCAN_PARAM scanparam = {1,0x30,0x06,0,0};//¹ã²¥²ÎÊý
...
/*param1.scanparam = iot_os_malloc(sizeof(T_OPENAT_BLE_SCAN_PARAM));
    ...
    iot_ble_iotctl(0,BLE_SET_SCAN_PARAM,param1);//ÉèÖÃ¹ã²¥²ÎÊý
    ...
    if(msg->eventid != OPENAT_BLE_SET_SCAN_ENABLE)// µè´ýé¨ãèê¹äü³é¹¦
    {
        if(msg != NULL)
            iot_os_free(msg);
        msg = NULL;
...
BOOL ble_data_trans(VOID)
{
    ...
    /*Á italingua gent ом · ïö ° üº ЬØÄ † ìØ Pronunciation от £ ¢² ¢ "long will*/
    iot_ble_iotctl(connect_handle,BLE_FIND_SERVICE,param1);// · ¢ ïö · þîñ
    
    param2.uuid = iot_os_malloc(sizeof(T_OPENAT_BLE_UUID));
    memcpy(param2.uuid,&uuid_s,sizeof(T_OPENAT_BLE_UUID));
    iot_ble_iotctl(connect_handle,BLE_FIND_CHARACTERISTIC,param2);// · ¢ · · ^ þî-werage.
    if(param2.uuid != NULL)
        iot_os_free(param2.uuid);
    param2.uuid = NULL;
    iot_os_wait_message(ble_test_handle,&msg);
    if(msg->eventid != OPENAT_BLE_FIND_CHARACTERISTIC_IND)// µè´ý · ¢ ïöìøõ ÷ ³é¹¦
    {
        if(msg != NULL)
            iot_os_free(msg);
        msg = NULL;
    ...
    iot_ble_iotctl(connect_handle,BLE_OPEN_NOTIFICATION,param3);//Ò¿nnnnn
    if(param3.uuid != NULL)
