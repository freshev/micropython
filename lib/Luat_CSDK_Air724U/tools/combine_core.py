...
class CpioConfig():
    ...
    def _gen_file_data(self, local, remote) -> bytes:
        ...
        name_size = len(name) + 1  #includes null
...
if __name__ == "__main__":
    ...
    #Search for the underlying firmware
    for i in os.walk(core_dir):
        if i:
            core_name = i[2][0] 
            break
    #Exit if the required file is not retrieved in two seconds
    for i in range(20):
	...
	#Use parameter 4 to determine whether to replace sffs.img
    if sys.argv[4] != "NULL":
