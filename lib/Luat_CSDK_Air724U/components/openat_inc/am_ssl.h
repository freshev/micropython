...
typedef struct
{
    ...
    entropy_context          *ent;
	/*+\BUG\wj\2020.2.2.19\äóä´´´sSSSLE» ue¶1*/
    x509_crt                *ca;
    x509_crt                *user_ca;
    pk_context              *private_key;
    /*-\BUG\wj\2020.2.1 19\9*/
    
    kal_uint16               app_str;

    /* context state */
    #define TLS_CTX_INUSE    (0x0001)
    kal_uint16               state; // ê · ñõ¹ ú ¹ ¹ ¹ ¹ ¹
...
typedef struct {
    ...
    kal_uint32          cert_state; //ÔÝÊ±Î´ÓÃ£¬ÖµÎª0 /* handling cert with mmi_certman or certman */

    /* invalid certificate */
    kal_bool            check_cert; // Ö »Éèöãeëöµ £ ¬² ¢ Ã» óðÊµ¼ÊÊ¹ÓÃ
    ...
    kal_uint32          peer_cert_len;    // oodî ;the
