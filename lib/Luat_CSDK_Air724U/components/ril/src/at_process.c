...
static void processLine(const char *line)
{
    ...
    else if (isFinalResponseError(line))
    {
        ...
        case NUMERIC:
            ...
            break;
            /*+\ Newreq Atmmi-144 \ Xiongjunqun \ 2012.01.05 \ ðþääêösalandñnø´uâë² »ºïànµäø ½ ½ --- Öøðâð¸¸äibortedµäéï ± ± ±*/
        case SINGLELINE:
            LOGD("%s", __FUNCTION__);
			...
			else if((sp_response->finalResponse == NULL)&&(strstr(s_responsePrefix, "+CMGR") || strstr(s_responsePrefix, "+CMGL"))) /*½ "*/
			{
				addIntermediate(line);
			}
            else
        ...
        case MULTILINE:
            ...
            break;
            /*-\ Newreq Atmmi-144 \ Xiongjunqun \ 2012.01.05 \ ðþääêöquañnø´uâë² »ºïànµächa · ½ --- Öøðâþääabortedµäéï ± ± ±*/
...
VOID at_message(UINT8 *pData, UINT16 length)
{
    ...
    {
        pMessage = (RILChannelData *)iot_os_malloc(sizeof(RILChannelData));

        if (NULL != pMessage)
        {
            //copy data
            pMessage->data = (char *)iot_os_malloc(length);
...
static int writeline(const char *s, ATCommandType type, const char *responsePrefix, const char *smspdu)
{
    ...
    sc = (int)(IVTBL(enter_critical_section)());
