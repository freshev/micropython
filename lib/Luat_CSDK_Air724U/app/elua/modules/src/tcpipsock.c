...
static int l_sock_recv(lua_State *L) {
    ...
    char* buf;
    int recved_len = 0;
	/*+\wj\2020.1.22\BUG4307 UDPÊý¾ÝÊÕ²»È«*/
	int recv_left = 0;
    sock_index     = luaL_checkinteger(L, 1);
...
static int l_sock_setopt(lua_State *L) {
	...
	{
		/**Ä¿ç ° ¶ô½e¹ìåààðnµä²îêýéèöãã »× Ö´¦àn**/
		setret = -1;
	}

	/*Levelä -ç ° »Ö» Ö§³ösol_socketzy ippproto_tcp Name Ö§³öso_KeePalive £ ¬Tcp_Keepeppel £ ¬TCP_KEEPINTVL £ ¬TCP_KEEPCNT £ ¬SO_REEADDR*/
	if(sock_index < 0 
