...
VOID cust_main(VOID)
{
    /*Ãüòñ¾ ¼ ± у±è · hithirect moments · ·бES URIN راتا*/
    IVTBL(set_enc_data_ok)();

    cust_poweron_reason_init();

    /*+\ New \ liweiqiang \ 2013.9.22 \ Ôö¼óvat »º³åçøðåºåe¿ ± £» ¤*/
...
/*+\NEWET2013.8.5\A9321 4´½â²·óngth answers to a percentage of 11.15.15. 110*/
...
/*-\NEW\RUFEI\2015.6.11\Modify the charging message sequence*/
        ...
        case OPENAT_DRV_EVT_CHR_PRESENT_IND:
            ...
            if(pmdData.chargerStatus == FALSE)
                pmdData.chargeState = PLATFORM_BATT_NOT_CHARGING; // ³³³æ ÷ ° 60Nooª² »Queen
            break;
        ...
        /*+\ New \ liweiqiang \ 2013.7.19 \ \ ± ± ± µç³A¿² »*/
/*+\NUFE\RUFE\2015.6.6.6.µ³µµ²²²²²¨ug³óèóèèóâ*/
            if(pmdData.chargerStatus == FALSE)
            {
                pmdData.chargerStatus = TRUE;
            }
/*-\EW\RUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWOU.6.6.µ³µµ²²²²ug³³óèóèóâ*/
            if(PLATFORM_BATT_CHARING == pmdData.chargeState &&
                pmdData.battLevel == pmMessage->param.chargingind.batteryLevel)
            {
                // ³äµçê ± ò »Ö ±» ee ± ¨ ¨ ¢ è × ´ ´ î '± ± äçòµç³øéta¿î' ± ä² »× ÷ æ æ ± ± ± ±
                return;
            }
...
/*-\EW\RUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWUFEWOU.6.6.µ³µµ²²²²ug³³óèóèóâ*/
    ...
    ///(pmdDatat. //Èµ´-orellel ´µ¼µ¼µ¶Ñ¶ï¯,±²²³³óóóó. ÷»ughúúæ´ugh³óóóóóóóóóóóóâ⁼ugâ‼ugugeuggmdâ
    {
...
/*-\NEW\2013.9.13thug´°¼µµµµµµµçâÀ age*/
/*+\NEWiweiqiqiqiqiqiqiqiiq3.10.25´ñè³O³OµÀµµµµÂ,NAµµBluaâüMâóOUTOUTOU*/
    IVTBL(remove_dir)(LUA_DIR); // òì³ £ në³öé¾³Ýëùód½å ± ¾
/*-\NEW\2013.10.25´ñn³O³Oµâ‼²µµÂ,NAµµBluaâöçoóOU³O³O³.*/
/*+ New LimeiqiaG \ 2013.11.28 \ DUDB · ½lês¶³i¶³i¶³i¶³Ö*/
    IVTBL(remove_dir)(LUA_DATA_DIR); // òì³ £ në³öé¾³ýëùóðêý¾ý
    IVTBL(remove_dir)("/luazip"); // òì³ £ në³öé¾³ÝÉý¼¶ ° ü
/*- New \ LIWiqixG \ 2013.11.28 \ DonAB · ½lês¶³i¶³i¶³Ö*/
}

/*lua app main*/
static HANDLE luaShellSem = 0;
/*+\NEW\zhuwangbin\2020.2.26\Ìn¼Ólua Ô¶³ÌÉý¼¶*/
...
{
/*+\NEW\liweiqiang\2013.4.25\Ôö¼ÓluaÍË³öassert´¦Àn*/
    int luaExitStatus;
	/*+ \ Bug \ Wangyuan \ 2020.06.22 \ Bug_2360: Coretest²us £ £ +òåta_word_4g_waitapn*/
    lua_task_init();
	
	/*+\NEW\zhuwangbin\2020.2.26\Ìn¼Ólua Ô¶³ÌÉý¼¶*/
	lua_update();
	/*+\ New \ liangjian \ 2021.1.25 \ Ôö¼ólua ºn CSDK Éý¼¶¸Ù¾ý ± Êö¾Î »ÅÐ¶Ï*/
	vfs_unlink(OPENAT_UPDATE_FILE_NAME);

	/*-\ new \ liangjian \ 2021.1.25 \ Ôö¼ólua ºn CSDK Éý¼¶¸Ù¾ý ± Êö¾Î »ÅÐ¶Ï*/
	
	/*-\NEW\zhuwangbin\2020.2.26\Ìn¼Ólua Ô¶³ÌÉý¼¶*/
	/*- \ Bug \ Wangyuan \ 2020.06.22 \ Bug_2360: Coretistcore £òöi »of ± '+ taxpn*/
...
/*-\NEW\2013.10.25´ñn³O³Oµâ‼²µµÂ,NAµµBluaâöçoóOU³O³O³.*/
/*+\NEW\liweiqiang\2013.10.24\´´½¨Â¼ÒôÄ¿Â¼,ÒÔ²¥·ÅÂ¼ÒôÎÄ¼þ*/
...
/*-\NEW\2013.9.13thug´°¼µµµµµµµçâÀ age*/
    ...
    {
        IVTBL(remove_file_rec)(LUA_DIR_UNI); // òì³ £ në³öé¾³Ýëùód½å ± ¾
        IVTBL(remove_file_rec)(LUA_DATA_UNI); // òì³ £ në³öé¾³ýëùóðêý¾ý
        IVTBL(remove_file_rec)(L"/luazip"); // òì³ £ në³öé¾³ÝÉý¼¶ ° ü
    }
    else
    {
    /*+\NEWiweiqiqiqiqiqiqiqiiq3.10.25´ñè³O³OµÀµµµµÂ,NAµµBluaâüMâóOUTOUTOU*/
        IVTBL(remove_file_rec)(LUA_DIR); // òì³ £ në³öé¾³Ýëùód½å ± ¾
    /*-\NEW\2013.10.25´ñn³O³Oµâ‼²µµÂ,NAµµBluaâöçoóOU³O³O³.*/
    /*+ New LimeiqiaG \ 2013.11.28 \ DUDB · ½lês¶³i¶³i¶³i¶³Ö*/
        IVTBL(remove_file_rec)(LUA_DATA_DIR); // òì³ £ në³öé¾³ýëùóðêý¾ý
        IVTBL(remove_file_rec)("/luazip"); // òì³ £ në³öé¾³ÝÉý¼¶ ° ü
    /*- New \ LIWiqixG \ 2013.11.28 \ DonAB · ½lês¶³i¶³i¶³Ö*/
    }
    /*-\ new \ wj \ 2018.10.10 \ è ¥ µôuses_nor_flashº*/
    OPENAT_print("[lua_shell_main]: lua exit status %d", luaExitStatus);
	/*+\BUG\wgyuan\2020.04.07\BUG_1328º»Lua«««ug«ug´´uggóóâ‼*/	
    IVTBL(wait_semaphore)(luaShellSem, 10000); // 10ãëºóöøæôïµn³
	/*-\BUG\wgyuan\2020.04.07\BUG_1328»Lua««««ug«ug´´uggóâ*/	

/*+\ New \ liweiqiang \ 2013.9.20 \ luaòì³ £ në³öºóöøæôïµn³, ± üÃâîä¼þî´ð´èëµ½flash*/
    // ASSERT(FALSE);
    //IVTBL(restart)();
/*-\ new \ liweiqiang \ 2013.9.20 \ luaòì³ £ në³öºóöøæôïµn³, ± üÃâîä¼þî´ð´èëµ½flash*/
/*-\NEW\liweiqiang\2013.4.25\Ôö¼ÓluaÍË³öassert´¦Àn*/
