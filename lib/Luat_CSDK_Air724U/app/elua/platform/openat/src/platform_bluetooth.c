...
static void bluetooth_callback(T_OPENAT_BLE_EVENT_PARAM *result)
{
	...
	case OPENAT_BLE_RECV_DATA:
		if ((result->len != 0) && (result->dataPtr != NULL))
		{
			...
			/*+\NEW\czm\2020.11.27\´úÂëÆÀÉó: ²åÈë¶ÓÁÐÇ°ÅÐ¶ÏÊ£Óà¿Õ¼ä£¬¼ÓÁÙ½çÇø±£»¤*/
			HANDLE crihand = OPENAT_enter_critical_section();
			// Ihnai £ ó ó óððö ó ó ó ó ó ó ó óà¿¿¿à¼¼¼¼¼) irg × ¹ ° £ ° £ ° ° ×
...
{
	static plat_ble_recv_buff recv_buff = {0}; //±£´æÒ»´Î¶ÁÈ¡µÄËùÓÐÊý¾Ý
	static UINT8 buf_index = 0;				   // ± £ ´ ’name question
	...
	if ((buf_index == recv_buff.len) && recv_buff.len != 0) / // ± ± çhi ýêý ° üäuni¡ eye. e'ja
	{
		OPENAT_print("DRV_BT:  platform_ble_recv (buf_index == recv_buff.len) && recv_buff.len != 0");
		recv_buff.len = 0;
		buf_index = 0; // öø × ° «òæ
		return 0;	   / Srl TRA TORTO »¸öo3 £ §¾ë and from £ organic from £ Ühènoëmäz have ~ üÖils € Üöêt ~ ° °
	}
	...
	if (recv_buff.len == 0)
	{
		...
		int status = QueueDelete(&ble_recv_queue, (uint8 *)&recv_buff, sizeof(plat_ble_recv_buff)); //¶ÁÏÂÒ»°üÊý¾Ý
		buf_index = 0;																				// öø × ° «òæ
		if (status == 0)																			//¶ÓÁÐÖÐÃ»ÓÐÊý¾ÝÁË
			return 0;
	}

	UINT8 recvlen = 0; //Êµ¼ÊÄÜÈ¡µ½µÄÊý¾Ý°ü³¤¶È

	if ((recv_buff.len - buf_index) > data->len) / For ï¹¹ûâ £ ïž¹ääêäê italo¹³ý³³¤¶èr ã¹ »
		recvlen = data->len;					 // owise (it
	else										 :: // and practÂ L wares byýý³_³³¤¶³à herodoè¶ee¡μäè) § »èäêäê¡oë³
		recvlen = recv_buff.len - buf_index;	 //ÓÐ¶àÉÙÈ¡¶àÉÙ

	memcpy(data->dataPtr, recv_buff.dataPtr + buf_index, recvlen); //¿½±´Ö¸¶¨³¤¶ÈµÄÊý¾Ý
	buf_index += recvlen;										   // Ö¸ÏÒÏÖÔÚµÄÊÝ¾ÝÎ »ÖÃ

	data->uuid_flag = recv_buff.uuid_flag; //µ±Ç°Êý¾Ý°üµÄUUID±êÖ¾Î»

	if (recv_buff.uuid_flag == 0) // × ª'æμ ± ç ° uuid
