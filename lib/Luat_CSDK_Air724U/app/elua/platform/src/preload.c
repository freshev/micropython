...
static int decode_file(void *buf, size_t size, size_t count, FILE *fp)
{
    ...
    unsigned int act_low_boundary;  /*Òô512¶ôæëµä¶eè¡îä¼þµÄæÐÊ¼Î »ÖÃ*/
    unsigned int read_count;        /*Ðèòª´óîä¼þöð¶eè¡µä³¤È*/
    unsigned int act_up_boundary;   /*Òô512¶ôæëµä¶eè¡îä¼þµä½eêøî »ÖÃ*/
    unsigned int act_count;         /*¶ÁÈ¡µ½µÄÓÐÐ§Êý¾Ý³¤¶È*/
    unsigned char* temp;
    ...
    /*Eriâçorioânt "Ið¶enl½ ¹ıμý £ ì ìâμäliî from the ı ìraμî*/
    if(read_count > len)
    {
...
int decompress_file(const char *Comfilename,char *Decomdfilename)
{
    ...
    #define FILE_MAX_LEN  (250*1024)  /*¥¸ö they × × × ×öæ, î × ×öæöæ250k*/
    FILE *fp = NULL;
...
/*Ôning ± ²³ ³³³ ³³³ Lumed ×××× ziltμté ling always ai idle limit. 1*/
        ...
        else
        {
            ...
            {                    
                ...
                if(fwrite(pFileInfo->data, 1, pFileInfo->length, fout) != pFileInfo->length)
                {
                    ...
                    fclose(fout);
                    fout = NULL;
                    remove(filename); //Ð´ÎÄ¼þÊ§°Ü,É¾³ýÒÑÐ´µÄÎÄ¼þ.
                    err = LUADB_ERR_WRITE_FILE;
                    goto exit_err;
                }
...
int parse_BL_section_file()
{
    ...
    /*Assisi ¶¨ ide, ÷ ÷ ÷ ó ó óÃ μôneân ò³¾¶¶¶¶¶ï ± ± ± ± ± ± ± ± ± ± ± eodi¹ža ± ±³ºêžu¹¹¹¹¹¹¹¹¹¹¹¹¹¹¹¹¹¹¹*/
...
static int decodeFile(const u8 *pData, DbFileInfo *pFileInfo, u32 *pOffset)
{
    ...
    // ¼æ «òæ +îä îÃäênêèìììììììääiênêäèääääääiênêäääääengèèääääääääeng
...
/*+\ New \ Liweiqiang \ 2013.12.9 \ è ¥ µôluadb × îast*/
#if 0    
