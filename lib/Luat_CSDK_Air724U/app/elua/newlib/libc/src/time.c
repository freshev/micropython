...
#if 0 /*TIMES I'll youat "n mtil þï wμþμ³êμï*/
...
char* lualibc_asctime(const struct tm *_tblock)     /*Ôý² »Ö§³ö*/
{
    errno = ENOSYS;
    return NULL;
}

char* lualibc_ctime(const time_t *_time)            /*Ôý² »Ö§³ö*/
{
    errno = ENOSYS;
    return NULL;
}
...
int _isindst (struct tm *timeptr)    /*DST ïäeîê ± ôý² »Ö§³ö*/
{
    if(_daylight == 0)
        return 0;
...
struct tm* lualibc_localtime(const time_t *ptime)  /*Victory° ´´´t ´s COOR´´µ´´»*/
{
    struct tm *ptm;
    long ltime;
...
size_t lualibc_strfttime(char *_s, size_t _maxsize, const char *_fmt, const struct tm *_t)  /*Ôý² »Ö§³ö*/
{
    errno = ENOSYS;
    return 0;
}
