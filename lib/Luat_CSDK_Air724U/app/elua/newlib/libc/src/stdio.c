...
FILE *lualibc_fopen(const char *file, const char *mode)
{
	...
	/*+\ New \ zhuwangbin \ 2020.3.29 \ lua½uâö »ó × × × × ® µ_SFP ½ £ £*/
    if((fp = LUA__sfp()) == NULL)
        return (NULL);
	/*-\ new \ zhuwangbin \ 2020.3.29 \ lua½uâ »× × × × × ® µ_SFP ½ £ £ ¬ ¬ ¬ à ¬ ¬ã 8810 Newlibö µuµu¿ ú © ©*/
...
int lualibc_fclose(FILE *fp)
{
    ...
    // r = fp-> _ flags & __swr? __sflush (fp): 0; Óéóú² »ðèòªçå³ý» º´æ¹ÊÖ ± ½Ó · µ »ø0
...
{
    // óóóc »ó ó µ ï n µ µ ö ö àn §³ööpdin Ungetc
    ASSERT(fp != stdin);
...
size_t lualibc_fread(void *buf, size_t size, size_t count, FILE *fp)
{
    ...
    unsigned int act_low_boundary = (offset & 0xFFFFFE00); /*Òô512¶ôæëµä¶eè¡îä¼þµÄæÐÊ¼Î »ÖÃ*/
