...
/*+\NEW\liweiqiang\2013.5.16\ÐÞÕý²»ÍêÕûµÄ½âÑ¹ÎÄ¼þ²»»e±¨´nµÄÎÊÌâ*/
  if (thereIsSize)
  {
...
/*+ New \ LEWIQENG \ 2013.10.24 \ ÐÞiko¶Þ½øo¼ll*/
    fout = fopen(outfile, "wb");
/*- \ New \ LEWIQIANG \ 2013.124 \ riÞiko¶Þ½ Öνges*/
    if(!fout)
    {
    ...
    if(fwrite(outbuff, 1, outlen, fout) != outlen)
    {
        ...
        fclose(fout);
        fout = NULL;
        remove(outfile); //Ð´ÎÄ¼þÊ§°Ü,É¾³ýÒÑÐ´µÄÎÄ¼þ.
        ret = SZ_ERROR_FAIL;
        goto uncompress_b2f_exit;
    }
    fclose(fout);
					 ...
					 SizeT* outProcessed)
{
	...
	res = LUA_LzmaDec_DecodeToBuf(&lzmaDecState, pOutBuf, outProcessed,
		...
		/*+wzq\2014.10.30\Ñ¹Ëõ×îºó»e¶àÒ»¸ö×Ö·û*/
		lzmaUnpackSize -= *outProcessed;
		/*-Wzq \ 2014.10.30 ×× »» »» »» »¸ö ¸¸ × ö · û*/
