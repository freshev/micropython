...
int net_read(int fd, char *buf, size_t len)
{
	...
	if ( c == -1 )
	{
	    if ( socket_errno(fd) != OPENAT_EWOULDBLOCK )
		return -1;
	}
	else
	{
...
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl)
{
	...
	*pnum++ = '\0';
    ...
    free(lhost);
    iot_debug_print("[ftp] FtpConnect 4444 %x,%x",sin.sin_addr.s_addr,sin.sin_port);
    sControl = socket(PF_INET, OPENAT_SOCK_STREAM, 0);
    if (sControl == -1)
    {
    	if (ftplib_debug)
    	    perror("socket");
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir)
{
    ...
    {
    	memset(&sin, 0, l);
    	sin.in.sin_family = OPENAT_AF_INET;
    ...
    sData = socket(PF_INET,OPENAT_SOCK_STREAM,0);
    if (sData == -1)
    {
    	if (ftplib_debug)
    	    perror("socket");
