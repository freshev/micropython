...
typedef struct SmsEntityQueueTag
{
    ...
    u8 funFirst;                                
    u8 funLast;                                 
	char tNewSms[SMS_QUEUE_COUNT];				/*¶¶ ±'and*/
}SmsEntityQueue;
...
// _ _ odiLßе½μßμ¹μ¹μ¹μ¹¹μ¹¹μ¹¹τιªæe ·êe × تèOê
...
static int gsmInvertNumbers(const char* pSrc, char* pDst, int nSrcLength)
{
    ...
    for(int i=0; i<nSrcLength;i+=2)
    {
        ch = *pSrc++;        // ± £'ïè³öïöμä × ö Î
        *pDst++ = *pSrc++;   // ¸'öæº³³öïöμä × ö · û
        *pDst++ = ch;        // ¸'öæïè³³öïöμä × ö Î
    }
...
bool gsmDecodePdu(char* pdu, int pdulen)
{
	...
	offset = offset + 2;
	gsmString2Bytes(ppdu+offset+6, buf, 2);
	/*--ÓÃ»§ÐÅÏ¢±àÂë·½Ê½ Dcs=8£¬±nÊ¾¶ÌÐÅ´æ·ÅµÄ¸ñÊ½ÎªUCS2±àÂë*/
	dcs = buf[0];
	offset = offset + 2;
