...
static int binsearch(const uint16_t *sortedSeq, int seqLength, uint16_t keyData) {
    ...
    while (low <= high) {
        mid = (low + high)>>1;//Shift right by 1 bit is equal to /2, odd number, no matter odd or even, as long as there is a value
        if (keyData < sortedSeq[mid]) {
            high = mid - 1;//It is mid-1, because mid has been compared
        }
...
//Character height: 24
//internal font
//Use sorting and binary lookup table
