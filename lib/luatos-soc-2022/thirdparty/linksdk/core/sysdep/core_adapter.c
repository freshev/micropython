...
/** CORE_ADAPTER_MBEDTLS_ENABLED is not a compilation switch that users need to care about
 *
 * In most cases, just keep it as set below
 *Only a few times, SDK users care about the ROM size of the docking layer code, and do not choose to use TLS to connect to the server
 * Only then will the change of turning off the CORE_ADAPTER_MBEDTLS_ENABLED macro definition appear to reduce the docking size
 *
 * We do not recommend removing the line #define CORE_ADAPTER_MBEDTLS_ENABLED
 * Although the IoT platform accepts TCP connections, we do not recommend this. TLS is a more secure communication method.
 **/
