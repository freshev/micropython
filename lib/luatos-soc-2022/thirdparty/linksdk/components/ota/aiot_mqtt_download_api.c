 ...
 * @brief mqtt file download implementation file
 * @date 2019-12-27
 *
...
static int32_t _parse_json_header(void *handle, char *data, uint32_t data_len, aiot_mqtt_download_recv_t *pakcet)
{
    ...
    /*Parse file description information*/
    file_info = value;
...
static void _md_recv_data_reply_handler(void *handle, const aiot_mqtt_recv_t *msg, void *userdata)
{
    ...
    uint8_t *data = NULL;
    uint32_t data_len = 0;
    /*json header parsing variable definition*/
    char *header = NULL;
    ...
    /*Determine whether the received data offset is the requested offset. There may be repeated requests. If the verification fails here, the request will not be actively resent until it times out.*/
    if(packet.data.data_resp.offset != md_handle->size_fetched + md_handle->range_start) {
        return;
    }

    /*Generate callback packet callback user interface*/
    ...
    /*Data reception is successful, failure statistics are reset*/
    md_handle->failed_counter = 0;

    /*Call back the user interface and notify the stored data*/
    ...
    if(md_handle->size_fetched < md_handle->range_size) {
        ...
        /*The download is completed. If there is md5, the entire file needs to be verified.*/
...
int32_t aiot_mqtt_download_process(void *handle)
{
    ...
    case STATE_MQTT_DOWNLOAD_INIT: {
        /*The fragment download end is not set, and the download is set to the end of the file.*/
        ...
        /*Topic returned by subscription data*/
        _md_sub_response_topic(md_handle);
        /*Download the complete file and do md5 verification*/
