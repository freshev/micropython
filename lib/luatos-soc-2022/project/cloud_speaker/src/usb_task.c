...
void luat_uart_recv_cb(int uart_id, uint32_t data_len){
    ...
    else if (strstr(data_buff, "AT+CLIENTID=") != NULL)
    {
        ...
        if(flag != NULL)
        {
            ...
            if (flag2 != NULL)
            {
                int ret = luat_fskv_set("clientId", flag, strlen(flag) - 2);              //Subtract the \r\n at the end
                luat_uart_write(LUAT_VUART_ID_0, "OK", strlen("OK"));
            }
    ...
    else if (strstr(data_buff, "AT+USERNAME=")  != NULL)
    {
        ...
        if(flag != NULL)
        {
            ...
            if (flag2 != NULL)
            {
                int ret = luat_fskv_set("username", flag, strlen(flag) - 2);              //Subtract the \r\n at the end
                luat_uart_write(LUAT_VUART_ID_0, "OK", strlen("OK"));
            }
    ...
    else if (strstr(data_buff, "AT+PASSWORD=") != NULL)
    {
        ...
        if(flag != NULL)
        {
            ...
            if (flag2 != NULL)
            {
                int ret = luat_fskv_set("password", flag, strlen(flag) - 2);              //Subtract the \r\n at the end
                luat_uart_write(LUAT_VUART_ID_0, "OK", strlen("OK"));
            }
