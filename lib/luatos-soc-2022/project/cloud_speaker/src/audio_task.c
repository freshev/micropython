...
void audio_data_cb(uint8_t *data, uint32_t len, uint8_t bits, uint8_t channels)
{
    ...
    if(ret > 0)
    {
        // LUAT_DEBUG_PRINT("cloud_speaker_audio_task get volume success %d", value); //A lot of printing will come out when opened, which affects log viewing. You can open it yourself if necessary
        HAL_I2sSrcAdjustVolumn(data, len, value);
    }
    else
    {
        // LUAT_DEBUG_PRINT("cloud_speaker_audio_task get volume fail %d", value); //A lot of printing will come out when opened, which affects log viewing. You can open it yourself if necessary
...
void audio_task_init(void)
{
	...
	//luat_audio_play_set_bus_type(LUAT_AUDIO_BUS_SOFT_DAC); //Play through DAC pin
    ...
    int result = luat_rtos_task_create(&audio_task_handle, 2048, 20, "audio task", audio_task, NULL, NULL);
