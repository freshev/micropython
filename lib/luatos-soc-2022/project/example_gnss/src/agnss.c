...
/// @brief Hezhou IOT project productkey must be added, otherwise the positioning will fail.
...
/// @brief Convert string to BCD encoding
/// @param arr string input
/// @param len length
/// @param outPut output
/// @return
...
static void luatos_http_cb(int status, void *data, uint32_t len, void *param)
{
    ...
    case HTTP_STATE_SEND_BODY_START:
        // If it is POST, send the POST body data here. If it cannot be sent completely at one time, you can continue sending it in the HTTP_STATE_SEND_BODY callback.
        break;
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                luat_rtos_task_sleep(30 * 1000);            //Failed to obtain ephemeris, try again after 30s
            }
        }
    }
}
