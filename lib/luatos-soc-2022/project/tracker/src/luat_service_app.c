...
static void i2c_task_proc(void *arg)
{
	...
	//Configure the calculator sensor vibration interrupt pin
...
static void luat_mx3416_monitor_task(void *args)
{
	...
	while (1)
	{
		...
		{
			//If acc is broken but still in motion, alarm
			count++;
			if (0 == g_s_acc_status)
			{
				if (count > 20)
				{
					count = 0;
					device_is_stop = 1;
					LUAT_DEBUG_PRINT("Vibrate alarm! ! !");
...
// todo data supplementary transmission
...
{
	// BSP_SetPlatConfigItemValue(0, 0); // If the system crashes, it will print information instead of restarting.
	...
	luat_gpio_cfg_t gpio_cfg;
	//Configure acc access interrupt pin
	luat_gpio_set_default_cfg(&gpio_cfg);
