...
void BSP_LoadPlatConfigFromFs(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // Can't read 'plat_config' version:2428 file body, or body not right , ( 60 /  ) , use the defult value 
...
void BSP_SavePlatConfigToRawFlash(void)
{
    ...
    if(1 == getOSState())
    {
        ...
        if(BSP_QSPI_Write_Safe((uint8_t*)&platInfo, FLASH_MEM_PLAT_INFO_NONXIP_ADDR, sizeof(plat_info_layout_t)) != 0)
        {
            ECPLAT_PRINTF(UNILOG_PLA_DRIVER, BSP_SavePlatConfigToRawFlash_2, P_ERROR, "1 Save plat config to raw flash error!!!");
        }
    }
    else
#endif
    {
        ...
        {
            ECPLAT_PRINTF(UNILOG_PLA_DRIVER, BSP_SavePlatConfigToRawFlash_4, P_ERROR, "2 Save plat config to raw flash error!!!");
        }
    }
...
static void BSP_WriteToRawFlash(uint8_t* pBuffer, uint32_t bufferSize)
{
    ...
    if(1 == getOSState())
    {
        ...
        {
            ECPLAT_PRINTF(UNILOG_PLA_DRIVER, BSP_WriteToRawFlash_2, P_ERROR, "3 Save plat config to raw flash error!!!");
        }
    }
    else
#endif
    {
        ...
        {
            ECPLAT_PRINTF(UNILOG_PLA_DRIVER, BSP_WriteToRawFlash_4, P_ERROR, "4 Save plat config to raw flash error!!!");
        }
    }
