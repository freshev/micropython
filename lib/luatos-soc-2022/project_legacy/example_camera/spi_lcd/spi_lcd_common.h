...
typedef struct
{
	...
	uint8_t blk_pin;
	uint8_t direction;
    uint8_t x_offset;//Offset
    uint8_t y_offset;//Offset
}spi_lcd_ctrl_t;
...
typedef struct
{
	union
	{
		uint8_t *queue;	//If data exceeds 4 bytes, put the address
		uint8_t buf[4];	//If it does not exceed 4 bytes, put the value directly
	};
	uint8_t len;
	uint8_t cmd;
}spi_lcd_cmd_t;
