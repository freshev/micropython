        ...
        if (xQueueReceive(padKeyEventQueueHandle, &msgId, portMAX_DELAY))
        {
            ...
            case PWR_MESSAGE:
            {
                if (pdFALSE != xTimerIsTimerActive(timerHandlePower))
                {
                    ...
                    if (vbat > 4000)
                    {
                        char str[] = "Current battery level is high";
                    ...
                    {
                        char str[] = "Current battery is low";
                        currentPower.message.tts.data = malloc(sizeof(str));
                        memcpy(currentPower.message.tts.data, str, sizeof(str));
                        currentPower.message.tts.len = sizeof(str);
                    }
...
void key_pad_init(void)
{
    ...
    //Currently, key1 on the cloud speaker development board uses gpio, so it is temporarily used as a gpio interrupt. The agpio can only maintain the level during sleep and cannot be used for interrupts. The subsequent design is changed to wakeup pad
