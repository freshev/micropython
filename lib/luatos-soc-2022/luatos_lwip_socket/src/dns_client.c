...
{
    llist_head node;
    Buffer_Struct uri_buf;  //Static does not need to be released
...
{
    luat_dns_require_t *require = (luat_dns_require_t *)zalloc(sizeof(luat_dns_require_t));
    ...
    if (!process)
    {
        process = (dns_process_t *)zalloc(sizeof(dns_process_t));
...
NET_DNS_TX_IPV4:
        ...
        DBG("is_done:%d, session_id:%d, ipv6:%d, dns server%d, try %d", process->is_done, process->session_id, process->is_ipv6, process->dns_cnt, process->retry_cnt);
