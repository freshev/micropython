# Micropython for Hezhou 4G Cat.1 Air724U (RDA8910) module

Original module documentation at [OpenLuat](https://docs.openluat.com/air724ug/)  
Development board [Schematic and PCB](https://oshwhub.com/luat/air724ug_air530_rtk)  
Translated module documentation at [Docs](docs/)

<!--
## Test board 
![Air724UG test board](https://github.com/freshev/micropython/blob/master/ports/air724/docs/pinout.png)
-->
...
2. Make
   ```bash
   cd micropython/ports/air724
   configure.bat
   make.bat
   ```
3. Firmware `micropython_Air724_VERSION` file is in `micropython/ports/air724/version/` folder.
...
1. Install drivers from `Drivers` folder.  
   After connecting SoC to PC your should see at least three new COM ports with following descriptions:  
   `LUAT USB Device 0 Modem` (AT commands port)  
   `LUAT USB Device 1 AT` (alternative AT commands port in `AT firmware` or USB-UART COM port in `micropython`)  
   `LUAT USB Device 2 AP Diag` (Debug COM port)  

2. Get `Luatools` at [LuatOS](https://wiki.luatos.org/pages/tools.html)

3. Run Luatools_v3.exe in newly created folder.

4. Press `Download Firmware` button at upper right corner.

5. Press `Select the file` button and select `.pac` file from `micropython/ports/air724/version/` folder.

6. Press `download` button at `Luatools`.

7. The firmware download at `Luatools` should start.

or use `bat` files:

1. `burn_all.bat` burns all files from pac to SoC.  
   `burn_app.bat` burns only APP_IMG file from pac to SoC.  
2. If you have more than one COM port during burn, add -port XX to `burn_all.bat`/`burn_app.bat` arguments.  
   XX should be two digits - the COM port number with device name `SPRD U2S Diag` in Windows `Device Manager`:  
	```bash
	burn_app.bat -port 28
	```
3.  First time use `burn_all.bat` to prepare whole flash.  
    Next times use `burn_app.bat` to burn only micropython part. It is much faster.

## Debug

1. You can see debug logs via `Luatools`:  
   * Open `Luatools`.  
   * Go to `Account`->`Tool configuration`->`Log` and check all logs.  
   * In main window uncheck `4G trace by USB`, `Normal Serial Port Loging`, `HOST...`.  
   * Below select COM port with description `LUAT USB Device 2 AP Diag`  .
   * Push `Open serial port` button.  

2. Or you can use `cooltools` for RDA8910 from `.\lib\Luat_CSDK_Air724U\tools\win32\` folder.  
...
4. Upgrade pack `Air724_OLDVERSION_to_NEWVERSION` file is in `micropython/ports/air724/fota/` folder.
...
Note the COM port with description `LUAT USB Device 1 AT`
...
```bash
cli.exe --port COM24 put frozentest.py 
```
...
Now supports only modules `os` and `UART`. 
<!--
- [x] GPIO: `machine.Pin`
- [x] ADC: `machine.ADC`
- [x] PWM: `machine.PWM`
...
* The size of micropython heap is roughly 2Mb. 
...
* My Air724EG (GPS) module often does not boot correctly due to insufficient power from USB. 
